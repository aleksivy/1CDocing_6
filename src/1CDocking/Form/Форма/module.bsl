...
Процедура Формирование_Акт(App, pd, orgId, Документ, Бланк, ИД)
	...
	Попытка
		...
		Если ЗначениеЗаполнено(Документ.Сделка) Тогда
			...
			Если ИмяБазы = "БУ" Тогда	
				Если ТекстДоНомера = "Документ" Тогда
					 rsMain.Fields.Item("OSN_T").Value  =  Строка("Розр.док.:");
					 rsMain.Fields.Item("OSN_TIP").Value  =  Строка("Документ розрахунків з контрагентом (ручний облік)");
					 rsMain.Fields.Item("OSN_DATA").Value  =  Дата(Документ.Сделка.ДатаВходящегоДокумента); 
			         rsMain.Fields.Item("OSN_NOMER").Value  =  Документ.Сделка.Ссылка.НомерВходящегоДокумента;
				 ИначеЕсли ТекстДоНомера = "Реализация" Тогда
					 rsMain.Fields.Item("OSN_T").Value  =  Строка("Розр.док.:");
					 rsMain.Fields.Item("OSN_TIP").Value  =  Строка("Реалізація товарів і послуг");
					 rsMain.Fields.Item("OSN_DATA").Value  =  Дата(Документ.Сделка.Дата);
					 НомерБезПрефикса = ПолучитьНомерБезНулей(Документ.Сделка); 
			         rsMain.Fields.Item("OSN_NOMER").Value  =  НомерБезПрефикса;
				ИначеЕсли ТекстДоНомера = "Счет" Тогда
					 rsMain.Fields.Item("OSN_T").Value  =  Строка("Розр.док.:");
				 ...
				 Иначе 
					 rsMain.Fields.Item("OSN_T").Value  =  Строка("Розр.док.:");
					 rsMain.Fields.Item("OSN_TIP").Value  =  Строка("Акт про надання виробничих послуг");
					 rsMain.Fields.Item("OSN_DATA").Value  =  Дата(Документ.Сделка.Дата);
					 НомерБезПрефикса = ПолучитьНомерБезНулей(Документ.Сделка); 
			         rsMain.Fields.Item("OSN_NOMER").Value  =  НомерБезПрефикса;
				 КонецЕсли;
			Иначе //ИмяБазы = "УТП"	и ИмяБазы = "УПП" 
				 Если ТекстДоНомера = "Заказ" Тогда
					 rsMain.Fields.Item("OSN_T").Value  =  Строка("Замовлення:");
					 rsMain.Fields.Item("OSN_TIP").Value  =  Строка("Замовлення покупця");
					 rsMain.Fields.Item("OSN_DATA").Value  =  Дата(Документ.Сделка.Дата); 
					 НомерБезПрефикса = ПолучитьНомерБезНулей(Документ.Сделка); 
			         rsMain.Fields.Item("OSN_NOMER").Value  =  НомерБезПрефикса;
				 Иначе 
		...
		Если ЗначениеЗаполнено(Документ.ДоговорКонтрагента) Тогда
			...
			Если ИмяБазы = "УПП" или ИмяБазы = "УТП" Тогда
				rsMain.Fields.Item("DOG_NAME").Value = Документ.ДоговорКонтрагента.Наименование;
...
// Серна - Начало
				...
				Если НомерДокумента1С = НомерДокументаМед и Контрагент1С = КонтрагентМед Тогда
				...
				ИначеЕсли (Документ.Метаданные().Имя = "НалоговаяНакладная") и (ИдДокументаМед = "") и (Reestr.Fields.Item("SENDER").Value = 1) и (Reestr.Fields.Item("DOC_MOVE").Value = 1)
					...
					Если ЗначениеЗаполнено(ДатаРегистрации) и Документ.ВключенаВЕдиныйРеестрНалоговыхНакладных = Ложь Тогда
						Попытка
							...
							Документ1С.ВключенаВЕдиныйРеестрНалоговыхНакладных = Истина;
							Документ1С.ДатаРегистрации = НачалоДня(Reestr.Fields.Item("REGDATE").Value);
							Документ1С.НомерРегистрацииВЕРПН = СокрЛП(Reestr.Fields.Item("REG_NUM").Value);
							Документ1С.Записать( РежимЗаписиДокумента.Запись );
							лОшибка = Ложь;
						Исключение
							лОшибка = Истина;
						КонецПопытки;
					ИначеЕсли ЗначениеЗаполнено(ДатаРегистрации) и Документ.ВключенаВЕдиныйРеестрНалоговыхНакладных = Истина Тогда	
						Сообщить("Документ " +Документ+ " уже установлен признак фактической регистрации в ЕРНН.");
					КонецЕсли;
				КонецЕсли;
// Серна - Конец
...
// МодифицироватьФормуПриОткрытии
//
Процедура МодифицироватьФормуПриОткрытии()

    //**************************************************
    //ДОБАВЛЕНИЕ ЭЛЕМЕНТОВ ФОРМЫ
    //**************************************************
    ЭтаФорма.Панель.ТекущаяСтраница = ЭтаФорма.Панель.Страницы.Страница4;
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    Надпись = ЭлементыФормы.Добавить(Тип("Надпись"), "НадписьПерГраницыЗапретаИзмененияДанных", Истина);
    УстановитьПоложениеЭлемента(Надпись, 527, 19, 363, 196);
    ПолеВвода = ЭлементыФормы.Добавить(Тип("ПолеВвода"), "ПерГраницыЗапретаИзмененияДанных", Истина);
    УстановитьПоложениеЭлемента(ПолеВвода, 527, 19, 582, 102);
    //!!!возможно, в форме отсутствует реквизит ПерГраницыЗапретаИзмененияДанных, на который ссылается свойство "Данные"!!!!
    ПолеВвода.Данные = "ПерГраницыЗапретаИзмененияДанных";
    //**************************************************
    Надпись = ЭлементыФормы.НадписьПерГраницыЗапретаИзмененияДанных;
    Надпись.Заголовок = "Границы запрета изменения данных:";
    ...
    Надпись.УстановитьПривязку(ГраницаЭлементаУправления.Низ, ЭлементыФормы.НадписьПерГраницыЗапретаИзмененияДанных, ГраницаЭлементаУправления.Верх);
    Надпись.УстановитьПривязку(ГраницаЭлементаУправления.Право, ЭлементыФормы.НадписьПерГраницыЗапретаИзмененияДанных, ГраницаЭлементаУправления.Лево);
    //**************************************************
    ПолеВвода = ЭлементыФормы.ПерГраницыЗапретаИзмененияДанных;
    ПолеВвода.КнопкаВыбора = Истина;
    ПолеВвода.УстановитьДействие("ПриИзменении", Новый Действие("ПерГраницыЗапретаИзмененияДанныхПриИзменении"));
    ПолеВвода.УстановитьПривязку(ГраницаЭлементаУправления.Низ, ЭлементыФормы.ПерГраницыЗапретаИзмененияДанных, ГраницаЭлементаУправления.Верх);
    ПолеВвода.УстановитьПривязку(ГраницаЭлементаУправления.Право, ЭлементыФормы.ПерГраницыЗапретаИзмененияДанных, ГраницаЭлементаУправления.Лево);
    //**************************************************
	
	ЭтаФорма.Панель.ТекущаяСтраница = ЭтаФорма.Панель.Страницы.Страница1;
КонецПроцедуры // МодифицироватьФормуПриОткрытии

// УстановитьПоложениеЭлемента
//
...
КонецПроцедуры // УстановитьПоложениеЭлемента
// Серна - Конец
