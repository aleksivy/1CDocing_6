...
//		Иначе
//// Серна - Начало
//			Если (СокрЛП(ПоляНН.HKBUY) = "100000000000") или (СокрЛП(ПоляНН.HKBUY) = "300000000000") или (СокрЛП(ПоляНН.HKBUY) = "400000000000") или (СокрЛП(ПоляНН.HKBUY) = "600000000000") Тогда
//				rsMain.Fields.Item("EDR_POK").Value = "";
//			Иначе
...
//		КонецЕсли;
		...
		Для Каждого СтрокаТЧ Из СтруктураДанныеТЧ Цикл 
			...
			Если НЕ ЗначениеЗаполнено(Документ.ОсновнаяНалоговаяПриРезервеНомераПриПродажаНижеОбычнойЦены) Тогда
				Попытка
// Серна - Начало
					Если (СтрокаТЧ.G32 <> Неопределено) и (СтрокаТЧ.G32 <> "") Тогда 
// Серна - Конец
						rsMain.Fields.Item("TAB1_A132").Value = 1
...
// Серна - Начало
				Если НомерДока = НомерДокаМед и КонтрМед = Контр Тогда
					Если ИД = МедИд Тогда
// Серна - Конец
...
// Серна - Начало
					ИначеЕсли (Документ.Метаданные().Имя = "НалоговаяНакладная") и (МедИд = "") и (Reestr.Fields.Item("SENDER").Value = 1) и (Reestr.Fields.Item("DOC_MOVE").Value = 1)
