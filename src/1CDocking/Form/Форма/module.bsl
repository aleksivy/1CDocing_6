...
// Серна - Начало
	ИначеЕсли Найти(ИмяБазы, "ЗарплатаИУправлениеПерсоналомДляУкраины") = 1 Тогда ИмяБазы="ЗУП"
// Серна - Конец
	КонецЕсли;
	Возврат ИмяБазы;
...
// Серна - Начало
	...
	|	И (Контрагенты.КодФилиала = &КодФилиала
	|			ИЛИ Контрагенты.КодФилиала ПОДОБНО &КодФилиала + ""@%"")
// Серна - Конец
	|
	|УПОРЯДОЧИТЬ ПО
	|	КодФилиала";
...
// Серна - Начало
	Если Не ЗначениеЗаполнено(ДатаДог) или Не ЗначениеЗаполнено(НомерДог) Тогда
		Возврат Справочники.ДоговорыКонтрагентов.ПустаяСсылка();
	КонецЕсли;
	
	лЗапрос = Новый Запрос;
	лЗапрос.УстановитьПараметр("Контрагент", Контрагент);
	лЗапрос.Текст = "ВЫБРАТЬ Спр.Ссылка ИЗ Справочник.ДоговорыКонтрагентов КАК Спр ГДЕ НЕ Спр.ЭтоГруппа И Спр.Владелец = &Контрагент";
	Если ЗначениеЗаполнено(ДатаДог) Тогда
		лЗапрос.УстановитьПараметр("ДатаДог", ДатаДог);
		лЗапрос.Текст = лЗапрос.Текст + " И Спр.Дата = &ДатаДог ";
	КонецЕсли;
	Если ЗначениеЗаполнено(НомерДог) Тогда
		лЗапрос.УстановитьПараметр("НомерДог", НомерДог);
		лЗапрос.Текст = лЗапрос.Текст + " И Спр.Номер = &НомерДог ";
	КонецЕсли;
...
// Серна - Конец
	...
	Если ИмяБазы = "БУ" Тогда
		...
		КонецЕсли;
// Серна - Начало
		НайденныйДоговорОбъект.Номенклатура = Справочники.Номенклатура.НайтиПоНаименованию("інші професійні послуги");
		НайденныйДоговорОбъект.ТипДоговора = Перечисления.ТипДоговора.Административные;
// Серна - Конец
	...
	Иначе
// Серна - Начало
		НайденныйДоговорОбъект.ОбменДанными.Загрузка = Истина;
// Серна - Конец
...
Процедура ДоступносьПриложений()
	...
	ИначеЕсли ЭлементыФормы.ТипДокОтчета.Значение = "РегламентированныйОтчетДекларацияОПрибыли" Тогда
		 ...
		 ЭлементыФормы.Приложения_таб10.Видимость = Истина;
		 ЭлементыФормы.Приложения_таб10.Заголовок = "Додаток ПП до Податкової декларації з податку на прибуток підприємств";	
...
ИначеЕсли ЭлементыФормы.ТипДокОтчета.Значение = "РегламентированныйОтчетУточняющийРасчетДекларацияНДС" Тогда
		 ЭлементыФормы.ОтчНал.Значение = Истина;
		 ЭлементыФормы.ТаблицаОтчетов.Колонки.СтатусОтчетаДФС.Видимость = Истина;
		 ...
		 ЭлементыФормы.Приложения_таб8.Видимость = Истина;
		 ЭлементыФормы.Приложения_таб8.Заголовок = "Дод.9 Розрах. под. зобов. за операціями, визн. в ст.161 ЗУ 'Про держ. підтримку с\г України', та питом. ваги варт. с/г товарів (ДС9)";	
		
		ЭлементыФормы.РамкаГруппыТаб.Видимость = Истина;
		
	ИначеЕсли ЭлементыФормы.ТипДокОтчета.Значение = "ЗаявлениеРасчетФСС" Тогда
		ЭлементыФормы.ОтчФСС.Значение = Истина;
		ЭлементыФормы.ТаблицаОтчетов.Колонки.СтатусОтчетаФСС.Видимость = Истина;
	КонецЕсли
...
Процедура СбросНастроекТабПриложений()
		...
		ЭлементыФормы.Приложения_таб10.Видимость = Ложь;
...
Процедура Формирование_Акт(App, pd, orgId, Документ, Бланк, ИД)
	...
	Попытка
		...
		Иначе
			РуководителиКонтрагента = ФормированиеПечатныхФорм.ОтветственныеЛицаКонтрагента(Документ.Контрагент,Документ.Дата);
		КонецЕсли;
		// покажем банковские реквизиты основного счета организации
		Банк		= СведенияОПоставщике.Банк;
		МФО	 		= СведенияОПоставщике.МФО;
		...
		Если ЗначениеЗаполнено(Документ.Сделка) Тогда
			...
			Иначе
				Если ОпределитьДокументРасчетов(Строка(ТипЗнч(Документ.Сделка))) =  "Замовлення покупця" тогда
					rsMain.Fields.Item("OSN_T").Value  =  Строка("Замовлення:");
				Иначе
					rsMain.Fields.Item("OSN_T").Value  =  Строка("Рахунок:");
				КонецЕсли;
			КонецЕсли;
			rsMain.Fields.Item("OSN_TIP").Value  =  ОпределитьДокументРасчетов(Строка(ТипЗнч(Документ.Сделка))); 
			...
			rsMain.Fields.Item("OSN_DATA").Value  =  Формат(Документ.Сделка.Дата, "ДЛФ = Д"); 
		Иначе
		...
		Если ЗначениеЗаполнено(Документ.ДоговорКонтрагента) Тогда
			Если ЗначениеЗаполнено(Документ.ДоговорКонтрагента.НаименованиеДляПечати) Тогда
			...
			Иначе
				rsMain.Fields.Item("DOG_NAME").Value = СокрЛП(Документ.ДоговорКонтрагента.Наименование);
				...
				Если ЗначениеЗаполнено(Документ.ДоговорКонтрагента.Дата) Тогда
				    ...
				    rsMain.Fields.Item("DOG_DATE").Value = Формат(Документ.ДоговорКонтрагента.Дата,  "ДЛФ = Д"); 
				КонецЕсли;	
			КонецЕсли;
		...
		Если ИмяБазы = "УПП" Тогда 
			...
			rsMain.Fields.Item("SIDE_DIR_POS").Value = Строка(РуководителиКонтрагента.РуководительДолжность);
			rsMain.Fields.Item("SIDE_DIR_FIO").Value = Строка(РуководителиКонтрагента.РуководительПредставление);
		Иначе
...
Процедура Формирование_АктСвойШаблон(App, pd, orgId, Документ, Бланк, ИД)
	...
	Попытка
		...
		rsMain.Fields.Item("TAXSYSTEM").Value = ПолучитьСхемуНалогообложения(ДатаДокумента);		
		rsMain.Fields.Item("FIRM_ADR").Value  =  СокрЛП(СведенияОПоставщике.ЮридическийАдрес); 
		rsMain.Fields.Item("FIRM_OTV_FIO").Value = СокрЛП(Документ.ПредставительОрганизации);
		rsMain.Fields.Item("FIRM_OTV_POS").Value = СокрЛП(ДанныеПредставителя.Должность);
		
		Если ИмяБазы = "УПП" Тогда 
			РуководителиОрганизации = РегламентированнаяОтчетность.ОтветственныеЛицаОрганизаций(Документ.Организация,  Документ.Дата);
			rsMain.Fields.Item("FIRM_OTV_POS").Value = СокрЛП(РуководителиОрганизации.РуководительДолжность);  
			rsMain.Fields.Item("FIRM_OTV_FIO").Value = СокрЛП(РуководителиОрганизации.Руководитель); 
			//rsMain.Fields.Item("SIDE_OTV_FIO").Value  = ""; 
		Иначе
			РуководителиОрганизации = ОбщегоНазначения.ОтветственныеЛица(Документ.Организация,  Документ.Дата);
			rsMain.Fields.Item("FIRM_OTV_POS").Value = СокрЛП(РуководителиОрганизации.РуководительДолжность);  
			rsMain.Fields.Item("FIRM_OTV_FIO").Value = СокрЛП(РуководителиОрганизации.РуководительПредставление);
			//rsMain.Fields.Item("SIDE_OTV_FIO").Value = СокрЛП(Документ.Получил);
			//rsMain.Fields.Item("SIDE_OTV_POS").Value = ?(ПустаяСтрока(РуководителиКонтрагента.РуководительДолжность) И Документ.Контрагент.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицо, "Директор", РуководителиКонтрагента.РуководительДолжность);
			//rsMain.Fields.Item("SIDE_OTV_FIO").Value = РуководителиКонтрагента.РуководительПредставление;
			rsMain.Fields.Item("SIDE_OTV_POS").Value  =  СокрЛП(Документ.Контрагент.ОсновноеКонтактноеЛицо.Должность);
			rsMain.Fields.Item("SIDE_OTV_FIO").Value  =  СокрЛП(Документ.Контрагент.ОсновноеКонтактноеЛицо);
	
		КонецЕсли;	 	
		
		rsMain.Fields.Item("FIELD2").Value = Формат(Документ.Дата, "ДДДММГГГГ"); 
		rsMain.Fields.Item("SIDE_CD_K").Value = СокрЛП(СведенияОПокупателе.ПолноеНаименование);
		...
		rsMain.Fields.Item("SUMPDV").Value  =  ИтогНДС; 
		rsMain.Fields.Item("DOCSUM").Value  =  ИтогСуммаСНДС; 
		rsMain.Fields.Item("FIELD1").Value = СокрЛП(Документ.МестоСоставленияДокумента); 
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			rsMain.Fields.Item("TAB1_F2").Value = Строка(СокрЛП(ВыборкаДетальныеЗаписи.Наименование));
			Try 
				ЕдИзм = ВыборкаДетальныеЗаписи.Номенклатура.БазоваяЕдиницаИзмерения.Наименование; 
				Если ЗначениеЗаполнено(ЕдИзм) тогда
					ЕдиницаИзмерения = ЕдИзм;
			...
			Иначе
				rsMain.Fields.Item("TAB1_F5").Value = ВыборкаДетальныеЗаписи.Цена;//Добавил условие - так как при установленной отметке «Сумма вкл. НДС» в шаблон передавалась не верная цена
			КонецЕсли;
...
Процедура Формирование_АктСвойШаблон_AktRobit(App, pd, orgId, Документ, Бланк, ИД)
	...
	Попытка
		...
		Если ЗначениеЗаполнено(Документ.Сделка) Тогда
			rsMain.Fields.Item("RAH_NAME").Value  =  Строка("Рахунок на оплату покупцю"); 
			НомерБезПрефикса = ПолучитьНомерБезНулей(Документ.Сделка); 
			rsMain.Fields.Item("RAH_NUM").Value  =  НомерБезПрефикса;
			rsMain.Fields.Item("RAH_DATE").Value  =  Дата(Документ.Сделка.Ссылка.Дата); 
		Иначе
			rsMain.Fields.Item("RAH_NUM").Value  =  "";
			rsMain.Fields.Item("RAH_DATE").Value  =  ""; 
		КонецЕсли; 
		
		Если ЗначениеЗаполнено(Документ.ДоговорКонтрагента) Тогда   
			rsMain.Fields.Item("DOG_NUM").Value = СокрЛП(Документ.ДоговорКонтрагента.Ссылка.Номер);
			rsMain.Fields.Item("DOG_DATE").Value = Формат(Документ.ДоговорКонтрагента.Ссылка.Дата,  "ДЛФ = Д");
			rsMain.Fields.Item("DOG_NAME").Value = Документ.ДоговорКонтрагента.НаименованиеДляПечати;                      
		КонецЕсли;
		...
		rsMain.Fields.Item("DATE_DOC").Value = Формат(Документ.Дата, "ДДДММГГГГ");
		rsMain.Fields.Item("SIDE_CD_K").Value = СокрЛП(СведенияОПокупателе.ПолноеНаименование);
		...
		rsMain.Fields.Item("SUMPDV").Value  =  ИтогНДС; 
		rsMain.Fields.Item("DOCSUM").Value  =  ИтогСуммаСНДС; 
		rsMain.Fields.Item("CITY").Value = СокрЛП(Документ.МестоСоставленияДокумента); 
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			rsMain.Fields.Item("TAB1_F3").Value = ВыборкаДетальныеЗаписи.Количество;	
			rsMain.Fields.Item("TAB1_F4").Value = Строка(СокрЛП(ЕдиницаИзмерения));
			rsMain.Fields.Item("TAB1_F5").Value = ВыборкаДетальныеЗаписи.Цена;
			rsMain.Fields.Item("TAB1_F6").Value = ВыборкаДетальныеЗаписи.Количество * ВыборкаДетальныеЗаписи.Цена;
			rsMain.Fields.Item("TAB1_F7").Value =  (ВыборкаДетальныеЗаписи.Количество * ВыборкаДетальныеЗаписи.Цена) - ВыборкаДетальныеЗаписи.Сумма; 
			rsMain.Fields.Item("TAB1_F8").Value = ВыборкаДетальныеЗаписи.Сумма;			
			rsMain.Post();  		
			Контр  =  КОнтр + 1; 
			Если  Контр <> Н Тогда 
				rsMain.Append();  
		...
		//ПолучитьПодчиненныеДокументы(Документ);
		РезультатФункции = Новый Массив;
	Если Метаданные.КритерииОтбора.СвязанныеДокументы.Тип.СодержитТип(ТипЗнч(Документ))  Тогда
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
		                |	СвязанныеДокументы.Ссылка
		                |ИЗ
						|	КритерийОтбора.СвязанныеДокументы(&ЗначениеКритерияОтбора) КАК СвязанныеДокументы
		                |Где СвязанныеДокументы.Ссылка ССЫЛКА Документ.ТребованиеНакладная";
			
		Запрос.УстановитьПараметр("ЗначениеКритерияОтбора",Документ);
		РезультатЗапроса = Запрос.Выполнить();
		ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
		...
		КонецЦикла;
		пДокумент = ВыборкаДетальныеЗаписи.Ссылка;
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
		...
		|ГДЕ
		|	ТребованиеНакладнаяМатериалы.Ссылка  =  &пДокумент";
	...
	КонецЕсли;
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			rsMain.Edit(); 
				rsMain.Fields.Item("TAB2_F1").Value = Контр + 1;
				rsMain.Fields.Item("TAB2_F2").Value = Строка(СокрЛП(ВыборкаДетальныеЗаписи.Номенклатура));			
				rsMain.Fields.Item("TAB2_F3").Value = ВыборкаДетальныеЗаписи.Количество;	
				rsMain.Fields.Item("TAB2_F4").Value = Строка(СокрЛП(ВыборкаДетальныеЗаписи.ЕдиницаИзмерения));			
			rsMain.Post();  		
			Контр  =  КОнтр + 1; 
			Если  Контр <> Н Тогда 
				rsMain.Append();  
...
Процедура Формирование_АктСвойШаблон_AKT_PS(App, pd, orgId, Документ, Бланк, ИД)
	...
	Попытка
		...
		rsMain.Fields.Item("TAXSYSTEM").Value = ПолучитьСхемуНалогообложения(ДатаДокумента);		
		rsMain.Fields.Item("FIRM_ADR").Value  =  СокрЛП(СведенияОПоставщике.ЮридическийАдрес); 
		rsMain.Fields.Item("CITY").Value = "Київ";
		rsMain.Fields.Item("PIDSTAVA").Value = "Статуту";
		
		Если ИмяБазы = "УПП" Тогда 
			РуководителиОрганизации = РегламентированнаяОтчетность.ОтветственныеЛицаОрганизаций(Документ.Организация,  Документ.Дата);
			rsMain.Fields.Item("FIRM_RUKPOS").Value = СокрЛП(РуководителиОрганизации.РуководительДолжность);
			rsMain.Fields.Item("FIRM_RUK").Value = СокрЛП(РуководителиОрганизации.Руководитель);
			rsMain.Fields.Item("FIRM_RUK_K").Value = СокрЛП(РуководителиОрганизации.Руководитель); 
			rsMain.Fields.Item("FIRM_BUHPOS").Value = "Головний бухгалтер";
			rsMain.Fields.Item("FIRM_BUH_K").Value = СокрЛП(РуководителиОрганизации.ГлавныйБухгалтер); 
			rsMain.Fields.Item("SIDE_OTV_FIO").Value  = Строка(РуководителиКонтрагента.РуководительПредставление);
			rsMain.Fields.Item("SIDE_RUKPOS").Value = Строка(РуководителиКонтрагента.РуководительДолжность);
			rsMain.Fields.Item("SIDE_RUK_K").Value = Строка(РуководителиКонтрагента.РуководительПредставление);
       ...
       КонецЕсли;	 	
		...
		ЕдиницаВПроизводстве = "РеализацияУслугПоПереработкеПродукция.ЕдиницаИзмерения"; 
		
		ИтогСуммаСНДС = Документ.СуммаДокумента;
		Если ТипЗнч(Документ) = Тип("ДокументСсылка.РеализацияУслугПоПереработке") Тогда
			ИтогНДС = Документ.Услуги.Итог("СуммаНДС");
		Иначе
			ИтогНДС = Документ.Товары.Итог("СуммаНДС") + Документ.Услуги.Итог("СуммаНДС");
		КонецЕсли;
		...
		rsMain.Fields.Item("SUMWITHOUTPDV").Value  =  ИтогСуммаСНДС - ИтогНДС;  
		rsMain.Fields.Item("SUMPDV").Value  =  ИтогНДС; 
		rsMain.Fields.Item("DOCSUM").Value  =  ИтогСуммаСНДС; 
		rsMain.Fields.Item("PLACE").Value = СокрЛП(Документ.МестоСоставленияДокумента); 
 ...
 //TAB1			             
		...
		|	ВЫБОР
		|		КОГДА РеализацияУслугПоПереработке.Ссылка.СуммаВключаетНДС  =  ЛОЖЬ
		|			ТОГДА РеализацияУслугПоПереработке.Цена
		|		ИНАЧЕ ВЫБОР
		|				КОГДА РеализацияУслугПоПереработке.СтавкаНДС  =  &Ставка20
		|					ТОГДА РеализацияУслугПоПереработке.Цена 
		...
		|	РеализацияУслугПоПереработке.ЕдиницаИзмерения КАК Единица,
		|	РеализацияУслугПоПереработке.ПроцентСкидкиНаценки КАК Скидка
		
		|ИЗ
		|	Документ.РеализацияУслугПоПереработке.Продукция КАК РеализацияУслугПоПереработке
		|ГДЕ
		|	РеализацияУслугПоПереработке.Ссылка  =  &Документ";
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			rsMain.Edit(); 
			rsMain.Fields.Item("TAB1_F1").Value = Контр + 1;
			rsMain.Fields.Item("TAB1_F2").Value = Строка(СокрЛП(ВыборкаДетальныеЗаписи.Номенклатура));
			Try 
				ЕдИзм = ВыборкаДетальныеЗаписи.Номенклатура.БазоваяЕдиницаИзмерения.Наименование; 
				Если ЗначениеЗаполнено(ЕдИзм) тогда
					ЕдиницаИзмерения = ЕдИзм;
			...
			rsMain.Fields.Item("TAB1_F3").Value = Строка(СокрЛП(ЕдиницаИзмерения));	
		...
		rsMain = Doc.DataSets("TAB1");
		  rsMain.Last();
		   Если ЗначениеЗаполнено(rsMain.Fields.Item("TAB1_F2").Value) Тогда
		  		rsMain.append();
		   КонецЕсли;
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			rsMain.Edit(); 
...
//TAB2
		...
		|ИЗ
		|	Документ.РеализацияУслугПоПереработке.ИспользованныеМатериалы КАК РеализацияУслугПоПереработке
		|ГДЕ
		|	РеализацияУслугПоПереработке.Ссылка  =  &Документ";
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			rsMain.Fields.Item("TAB2_F3").Value = Строка(СокрЛП(ЕдиницаИзмерения));	
			rsMain.Fields.Item("TAB2_F4").Value = ВыборкаДетальныеЗаписи.Количество;
			rsMain.Fields.Item("TAB2_F5").Value = ВыборкаДетальныеЗаписи.Цена;
			rsMain.Fields.Item("TAB2_F6").Value = ВыборкаДетальныеЗаписи.Сумма;			
			rsMain.Post();  		
			Контр  =  КОнтр + 1; 
			Если  Контр <> Н Тогда 
				rsMain.Append();  
...
Процедура Формирование_Счет(App, pd, orgId, Документ, Бланк, ИД)  		  
	...
	Попытка
		...
		СведенияОПокупателе  =  УправлениеКонтактнойИнформацией.СведенияОЮрФизЛице(Документ.Контрагент,  Документ.Дата, , , КодЯзыкаПечать);
		Если ЗначениеЗаполнено(Документ.СтруктурнаяЕдиница) Тогда
			Банк		= Документ.СтруктурнаяЕдиница.Банк;
			МФО	 		= Банк.Код;
			НомерСчета 	= Документ.СтруктурнаяЕдиница.НомерСчета;
		Иначе
...
Процедура Формирование_РасхНакл(App, pd, orgId, Документ, Бланк, ИД)
	...
	Попытка
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			rsMain.Fields.Item("TAB1_A5").Value = ВыборкаДетальныеЗаписи.ЦенаБезНДС;
			rsMain.Fields.Item("TAB1_A6").Value = ВыборкаДетальныеЗаписи.Сумма;
			rsMain.Fields.Item("TAB1_A4").Value = (ВыборкаДетальныеЗаписи.Количество * ВыборкаДетальныеЗаписи.ЦенаБезНДС) - ВыборкаДетальныеЗаписи.Сумма;
			//rsMain.Fields.Item("TAB1_A4").Value = ВыборкаДетальныеЗаписи.Скидка;;
			ZNYGKA = ZNYGKA + rsMain.Fields.Item("TAB1_A4").Value;//суммируем скидку по всем строкам
...
Процедура Формирование_АктаСверки(App, pd, orgId, Документ, Бланк, ИД)
	...
	Попытка
		...
		Если Документ.ДоговорКонтрагента <> Справочники.ДоговорыКонтрагентов.ПустаяСсылка() Тогда
			...
			ИначеЕсли  ИмяБазы = "БУ" Тогда
				Если ЗначениеЗаполнено(Документ.ДоговорКонтрагента.НаименованиеДляПечати) Тогда
					rsMain.Fields.Item("DOG_NAME").Value = Документ.ДоговорКонтрагента.НаименованиеДляПечати;
					rsMain.Fields.Item("NUM_DOG").Value = "";
			        rsMain.Fields.Item("DATA_DOG").Value = "";
				Иначе
					rsMain.Fields.Item("DOG_NAME").Value = СокрЛП(Документ.ДоговорКонтрагента.Наименование);
	                rsMain.Fields.Item("DOG_NUM").Value = СокрЛП(Документ.ДоговорКонтрагента.Номер);
				    rsMain.Fields.Item("DOG_DATE").Value = ?(Документ.ДоговорКонтрагента.Дата <> Дата(1,1,1),СокрЛП(Документ.ДоговорКонтрагента.Дата),"");
		...
		Иначе  
			...
			rsMain.Fields.Item("VO_PIB").Value  =  ?(ЗначениеЗаполнено(Документ.ПредставительОрганизации), СокрЛП(Документ.ПредставительОрганизации), "");
			rsMain.Fields.Item("SIDE_OTV_FIO_S").Value  = ?(ЗначениеЗаполнено(Документ.ПредставительКонтрагента), СокрЛП(Документ.ПредставительКонтрагента), "");
		КонецЕсли;	
...
Функция Формирование_НН(App, Документ, Бланк, PDate, ИД, orgId)  
	...
	Попытка
		...
		Попытка	
// Серна - Начало
		//Пока определим три причины, когда будет проставляться "Х" в графу N13 ("не выдается покупателю")
		Если (Документ.ТипПричиныНевыдачиПокупателюПриПродажеНижеОбычнойЦены = 15)или
			(Документ.ТипПричиныНевыдачиПокупателюПриПродажеНижеОбычнойЦены = 17)или 
			(Документ.ТипПричиныНевыдачиПокупателюПриПродажеНижеОбычнойЦены = 16) Тогда
			rsMain.Fields.Item("N13").Value  =  1;
		Иначе
...
// Серна - Конец
		...
		КонецЕсли;
// Серна - Начало
		Если ((Документ.ТипПричиныНевыдачиПокупателюПриПродажеНижеОбычнойЦены = 15) или (Документ.ТипПричиныНевыдачиПокупателюПриПродажеНижеОбычнойЦены = 17) или 
			(Документ.ТипПричиныНевыдачиПокупателюПриПродажеНижеОбычнойЦены = 16) или (Документ.ТипПричиныНевыдачиПокупателюПриПродажеНижеОбычнойЦены = 01) или
			(Документ.ТипПричиныНевыдачиПокупателюПриПродажеНижеОбычнойЦены = 12)) и ЗначениеЗаполнено(Документ.ОсновнаяНалоговаяПриРезервеНомераПриПродажаНижеОбычнойЦены) Тогда
			rsMain.Fields.Item("N3").Value   = СокрЛП(ПоляНН.HNAMESEL);
			rsMain.Fields.Item("N4").Value   = СокрЛП(ПоляНН.HKSEL);
		Иначе
// Серна - Конец
...
// Серна - Конец
		...
		rsMain.Fields.Item("KS").Value  = СокрЛП(ПоляНН.HKS); 
		...
		Для Каждого СтрокаТЧ Из СтруктураДанныеТЧ Цикл 
			...
			Попытка
				Если НЕ (СокрЛП(СтрокаТЧ.G6) = "0" и Сведенная = 1) и ПреобразоватьСтрокуВЧисло(СтрокаТЧ.G6) <> 0 Тогда
		...
		Попытка
			rsMain.Fields.Item("A5_72").Value  =  СокрЛП(ПоляНН.R01G14);   //Ставка 14%
			rsMain.Fields.Item("A6_72").Value  =  СокрЛП(ПоляНН.R03G14);
...
Функция Формирование_Дод2(App,  Документ,  Бланк,  PDate,  ИД, orgId) 
	...
	Попытка
		...
		//А нужно ли это
		Если Документ.Видоперации = Перечисления.ВидыОперацийНалоговаяНакладная.ОблагаемыеОперации Тогда
			rsMain.Fields.Item("PZOB").Value = Неопределено;
		ИначеЕсли Документ.Видоперации = Перечисления.ВидыОперацийНалоговаяНакладная.ОсвобожденныеОперации Тогда
			rsMain.Fields.Item("PZOB").Value = 2;
		ИначеЕсли Документ.Видоперации = Перечисления.ВидыОперацийНалоговаяНакладная.НеНДСОперации Тогда
			rsMain.Fields.Item("PZOB").Value = 1;
		...
		Если НЕПередаватьРеквизитыДоговора = Ложь Тогда
			 Попытка
				...
				КонецЕсли;
				Если ЗначениеЗаполнено(Документ.ДоговорКонтрагента.Ссылка.Номер) тогда
		...
		Для Каждого СтрокаТЧ Из СтруктураДанныеТЧ Цикл			
			...
			Если СтрокаТЧ.G001 <> 0 Тогда
				//rsMain.Fields.Item("TAB1_A01").Value = СокрЛП(СтрокаТЧ.G001); //№ з/п рядка
				  rsMain.Fields.Item("TAB1_A01").Value = СтрЗаменить(СтрокаТЧ.G001,Символы.НПП,"");
			Иначе
			...
			КонецПопытки;	
			Попытка
				Если СтрокаТЧ.G32 <> Неопределено Тогда // Ознака импорта
					Попытка
						rsMain.Fields.Item("TAB1_A32").Value = ?(ЗначениеЗаполнено(СтрокаТЧ.G32), 1, 0);
					Исключение	
					КонецПопытки;	
				КонецЕсли;//End Ознака импорта
...
// Серна - Начало
				...
				АббревиатураЕИ=СокрЛП(СтрокаТЧ.G4S);  //Alex
				Если Прав(АббревиатураЕИ,1)="." Тогда
					rsMain.Fields.Item("TAB1_A4").Value = Лев(АббревиатураЕИ,СтрДлина(АббревиатураЕИ)-1);//берем слева строку от точки
				Иначе
				rsMain.Fields.Item("TAB1_A4").Value  =  СокрЛП(СтрокаТЧ.G4S);  //Ед изм
				КонецЕсли;
// Серна - Конец
...
Процедура Формирование_1ПВКв(App,  Документ,  БланкДСС,  PDate,  ИД)
	...
	Если БланкДСС="S0301115" Тогда
		...
		Исключение
			rsMain.Fields.Item("NOMER").Value ="1"; 
		КонецПопытки;
		rsMain.Fields.Item("N9").Value = СокрЛП(Док["ПолеДляОчистки31"]);
		rsMain.Fields.Item("N11").Value = СокрЛП(Док["АдресПодразделение"]);	
...
Процедура Формирование_БалансМПЕО(App, Документ, БланкЕО, PDate, ИД)
	...
	Попытка
		...
		rsMain.Fields.Item("MY_DATE").Value = Документ.Дата;//Док.ДатаБаланса;
...
Процедура Формирование_ЗаявкаФСС(App, Документ, БланкЗаявкаФСС, PDate, ИД)
	Перем РабочаяДата;
	
	Doc = App.OpenOrCreateByExDocID(БланкЗаявкаФСС, ОпределитьКодПериода(Документ), PDate, , ИД, 0);
	РабочаяДата = Документ.ПериодРегистрации;
	
	rsMain = Doc.DataSets();
	НачалоОтчета(rsMain);
	
	rsMain.Fields.Item("FIRM_NAME").Value = Документ.Организация.НаименованиеПолное;
	
	СписокПоказателей = Новый СписокЗначений;
	СписокПоказателей.Добавить("","КодПоЕДРПОУ");	
	СписокПоказателей.Добавить("","НомерФССУ");
	КодПоЕДРПОУ = РегламентированнаяОтчетность.ПолучитьСведенияОбОрганизации(Документ.Организация, РабочаяДата, СписокПоказателей).КодПоЕДРПОУ;
	КодФСС 		= РегламентированнаяОтчетность.ПолучитьСведенияОбОрганизации(Документ.Организация, РабочаяДата, СписокПоказателей).НомерФССУ;
	
	rsMain.Fields.Item("FIRM_EDRPOU").Value = КодПоЕДРПОУ;
	rsMain.Fields.Item("CODE_FSSU").Value = КодФСС;
	...
	Запрос.УстановитьПараметр("Объект", Документ.Организация);
	...
	Если Выборка.Следующий() Тогда
		rsMain.Fields.Item("FIRM_ADR").Value =  Выборка.Адрес;
	КонецЕсли;
	...
	Выборка = Результат.Выбрать();
	Если Выборка.Следующий() Тогда
		Телефон = ПолучитьТелефонДляНалоговойНакладной(Выборка.Телефон);
		rsMain.Fields.Item("FIRM_TELORG").Value = Телефон;
	КонецЕсли;
	
	rsMain.Fields.Item("FIRM_FSSUACC").Value  =  Документ.Организация.БанковскийСчетДляРасчетовСФСС.НомерСчета;	
	rsMain.Fields.Item("FIRM_FSSUBANK").Value =  Документ.Организация.БанковскийСчетДляРасчетовСФСС.Банк.Наименование;
	rsMain.Fields.Item("FIRM_FSSUMFO").Value  =  Документ.Организация.БанковскийСчетДляРасчетовСФСС.Банк.Код;

	//СтрокаВремНетрудоспособность
	rsMain.Fields.Item("A1_3").Value = Документ.Больничные.Итог("Дни");
	rsMain.Fields.Item("A1_4").Value = Документ.Больничные.Итог("Сумма");
	Если Документ.Больничные.Итог("Дни") > 0 Тогда
		rsMain.Fields.Item("N16").Value = 1;
	КонецЕсли;
	rsMain.Fields.Item("A11_3").Value = Документ.Больничные.Итог("ДниЧАЭС");
	rsMain.Fields.Item("A11_4").Value = Документ.Больничные.Итог("СуммаЧАЭС");
	
	//СтрокаВремНетрудоспособностьБерем
	rsMain.Fields.Item("A2_3").Value = Документ.Декретные.Итог("Дни");
	rsMain.Fields.Item("A2_4").Value = Документ.Декретные.Итог("Сумма");
	Если Документ.Декретные.Итог("Дни") > 0 Тогда
		rsMain.Fields.Item("N17").Value = 1;
	КонецЕсли;
	rsMain.Fields.Item("A21_3").Value = Документ.Декретные.Итог("ДниЧАЭС");
	rsMain.Fields.Item("A21_4").Value = Документ.Декретные.Итог("СуммаЧАЭС");
	
	//СтрокаВремНетрудоспособностьПогребение 
	rsMain.Fields.Item("A3_3").Value = Документ.ПособияНаПогребение.Количество();
	rsMain.Fields.Item("A3_4").Value = Документ.ПособияНаПогребение.Итог("Сумма");
	Если Документ.ПособияНаПогребение.Количество() > 0 Тогда
		rsMain.Fields.Item("N18").Value = 1;
	КонецЕсли;
	
	//СтрокаВремНетрудоспособностьНС
	rsMain.Fields.Item("A4_3").Value = Документ.БольничныеНС.Итог("Дни");
	rsMain.Fields.Item("A4_4").Value = Документ.БольничныеНС.Итог("Сумма");
	Если Документ.БольничныеНС.Итог("Дни") > 0 Тогда
		rsMain.Fields.Item("N19").Value = 1;
	КонецЕсли;
	
	//Итог
	СуммаИтог = Документ.Больничные.Итог("Сумма") + Документ.БольничныеНС.Итог("Сумма") + Документ.Декретные.Итог("Сумма") + Документ.ПособияНаПогребение.Итог("Сумма"); 
	rsMain.Fields.Item("A7_4").Value = Документ.Больничные.Итог("Сумма") + Документ.БольничныеНС.Итог("Сумма") + Документ.Декретные.Итог("Сумма") + Документ.ПособияНаПогребение.Итог("Сумма");
	
	// подписи приложений
	Исполнитель = РегламентированнаяОтчетность.ОтветственныеЛицаОрганизаций(Документ.Организация, Документ.Дата, Документ.Ответственный);
	Если Документ.Больничные.Итог("Дни") > 0 ИЛИ Документ.Декретные.Итог("Дни") > 0 Тогда
		rsMain.Fields.Item("N1").Value = Исполнитель.ИсполнительДолжность;	
		rsMain.Fields.Item("N2").Value = Исполнитель.Исполнитель;	
		rsMain.Fields.Item("N3").Value = Телефон;	
	КонецЕсли;	
	Если Документ.ПособияНаПогребение.Количество() > 0 Тогда
		rsMain.Fields.Item("N4").Value = Исполнитель.ИсполнительДолжность;	
		rsMain.Fields.Item("N5").Value = Исполнитель.Исполнитель;	
		rsMain.Fields.Item("N6").Value = Телефон;	
	КонецЕсли;	
	Если Документ.БольничныеНС.Итог("Дни") > 0 Тогда
		rsMain.Fields.Item("N7").Value = Исполнитель.ИсполнительДолжность;	
		rsMain.Fields.Item("N8").Value = Исполнитель.Исполнитель;
		rsMain.Fields.Item("N9").Value = Телефон;
	КонецЕсли;
	
	rsMain.Fields.Item("REP_DATE").Value = Документ.Дата;	
	
	Руководители = ФормированиеПечатныхФорм.ОтветственныеЛица(Документ.Организация, Документ.Дата);

	Если ЗначениеЗаполнено(Руководители.Руководитель) Тогда
		rsMain.Fields.Item("FIRM_RUK").Value = Руководители.РуководительФИО.Фамилия + " " + Руководители.РуководительФИО.Имя	 + " " + Руководители.РуководительФИО.Отчество;	
	КонецЕсли;

	Если ЗначениеЗаполнено(Руководители.ГлавныйБухгалтер) Тогда
		rsMain.Fields.Item("FIRM_BUH").Value = Руководители.ГлавныйБухгалтерФИО.Фамилия + " " + Руководители.ГлавныйБухгалтерФИО.Имя + " " + Руководители.ГлавныйБухгалтерФИО.Отчество;	
	КонецЕсли;
	...
	// Приложение 1.1
	НомерСтроки = 1;
	РезультатЗапросаПоШапке = СформироватьЗапросПоШапке(Документ);
	...
	Если ВыборкаПоШапкеДокумента.Следующий() Тогда
		...
		Для Каждого Начисление ИЗ ТаблицаНачислений Цикл
			Если Не Начисление.РезультатФСС = 0 Тогда
				...
				rsMain.Fields.Item("TAB1_A1").Value = НомерСтроки;
				rsMain.Fields.Item("TAB1_A2").Value = Начисление.Фамилия;
				rsMain.Fields.Item("TAB1_A3").Value = Начисление.Имя;
				rsMain.Fields.Item("TAB1_A4").Value = Начисление.Отчество;
				rsMain.Fields.Item("TAB1_A5").Value = Начисление.ДРФО;
				rsMain.Fields.Item("TAB1_A6").Value = Начисление.ВидЗанятости;
				rsMain.Fields.Item("TAB1_A7").Value = Начисление.Серия;
				rsMain.Fields.Item("TAB1_A8").Value = Начисление.Номер;
				rsMain.Fields.Item("TAB1_A9").Value = Начисление.ЭтоПродолжениеБольничного;
				rsMain.Fields.Item("TAB1_A10").Value = Начисление.ПричинаНетрудоспособности.КодПричины;
				rsMain.Fields.Item("TAB1_A11").Value = Начисление.ПериодНачало;
				rsMain.Fields.Item("TAB1_A12").Value = Начисление.ПериодКонец;
				rsMain.Fields.Item("TAB1_A13").Value = Начисление.КоличествоДней;
				rsMain.Fields.Item("TAB1_A14").Value = Начисление.КоличествоДнейФСС;
				rsMain.Fields.Item("TAB1_A15").Value = Начисление.Результат;
				rsMain.Fields.Item("TAB1_A16").Value = Начисление.РезультатФСС;
				rsMain.Fields.Item("TAB1_A17").Value = Начисление.ДниЧАЭС;
				rsMain.Fields.Item("TAB1_A18").Value = Начисление.СуммаЧАЭС;
				rsMain.Fields.Item("TAB1_A19").Value = Начисление.НомерЧАЭС;
				rsMain.Fields.Item("TAB1_A21").Value = Начисление.СтажВМесяцах;
				rsMain.Fields.Item("TAB1_A22").Value = Начисление.СтажЗа12МесяцевВМесяцах;
	...
	Если ВыборкаПоШапкеДокумента.Следующий() Тогда
		...
		Пока ВыборкаПоДокументам.Следующий() Цикл 
			...
			rsMain.Fields.Item("TAB2_A1").Value = НомерСтроки;
			
			rsMain.Fields.Item("TAB2_A1_1").Value = 1; //получатель физ.лицо
			
			rsMain.Fields.Item("TAB2_A2").Value = ВыборкаПоДокументам.Фамилия;
			rsMain.Fields.Item("TAB2_A3").Value = ВыборкаПоДокументам.Имя;
			rsMain.Fields.Item("TAB2_A4").Value = ВыборкаПоДокументам.Отчество;
			rsMain.Fields.Item("TAB2_A5").Value = ВыборкаПоДокументам.ДРФО;
			rsMain.Fields.Item("TAB2_A6").Value = ВыборкаПоДокументам.ФамилияУмершего;
			rsMain.Fields.Item("TAB2_A7").Value = ВыборкаПоДокументам.ИмяУмершего;
			rsMain.Fields.Item("TAB2_A8").Value = ВыборкаПоДокументам.ОтчествоУмершего;
			rsMain.Fields.Item("TAB2_A9").Value = ВыборкаПоДокументам.СвидетельствоСерия;
			rsMain.Fields.Item("TAB2_A10").Value = ВыборкаПоДокументам.СвидетельствоНомер;
			rsMain.Fields.Item("TAB2_A11").Value = ВыборкаПоДокументам.Сумма;
	...
	Если ВыборкаПоШапкеДокумента.Следующий() Тогда
		...
		Для Каждого Начисление ИЗ ТаблицаНачислений Цикл
			Если Не Начисление.РезультатФСС = 0 Тогда
				...
				rsMain.Fields.Item("TAB3_A1").Value = НомерСтроки;
				rsMain.Fields.Item("TAB3_A2").Value = Начисление.Фамилия;
				rsMain.Fields.Item("TAB3_A3").Value = Начисление.Имя;
				rsMain.Fields.Item("TAB3_A4").Value = Начисление.Отчество;
				rsMain.Fields.Item("TAB3_A5").Value = Начисление.ДРФО;
				rsMain.Fields.Item("TAB3_A6").Value = Начисление.ВидЗанятости;
				rsMain.Fields.Item("TAB3_A9").Value = Начисление.Серия;
				rsMain.Fields.Item("TAB3_A10").Value = Начисление.Номер;
				rsMain.Fields.Item("TAB3_A11").Value = Начисление.ПричинаНетрудоспособности.КодПричины;
				rsMain.Fields.Item("TAB3_A12").Value = Начисление.ПериодНачало;
				rsMain.Fields.Item("TAB3_A13").Value = Начисление.ПериодКонец;
				rsMain.Fields.Item("TAB3_A14").Value = Начисление.КоличествоДнейФСС;
				rsMain.Fields.Item("TAB3_A15").Value = Начисление.РезультатФСС;
...
КонецПроцедуры //Формирование_ЗаявкаФСС
...
Функция ПолучитьТелефонДляНалоговойНакладной(Телефоны)
	...
	Если БланкДФС = "J0500105" ИЛИ БланкДФС = "F0500105" Тогда
		...
		Пока ДобавитьНовуюСтроку Цикл
			...
			Для Каждого Поле Из Док Цикл
				     ...
				     НомерБезПробелов = СтрЗаменить(Строка(НомерСтроки), Символы.НПП, "");
						   
				Если Найти(Поле.Ключ, "T1R"+Ноль+НомерБезПробелов+"G02") = 1 Тогда
					СуществуетСтрока = Истина;
					rsMain.Fields.Item("TAB1_A2").Value = Поле.Значение;
					
				ИначеЕсли Найти(Поле.Ключ, "T1R"+Ноль+НомерБезПробелов+"GFIO") = 1 Тогда
					СуществуетСтрока = Истина;
					rsMain.Fields.Item("TAB1_A21").Value = Поле.Значение;
					
				ИначеЕсли Найти(Поле.Ключ, "T1R"+Ноль+НомерБезПробелов+"G03A") = 1 Тогда
					СуществуетСтрока = Истина;
					rsMain.Fields.Item("TAB1_A31").Value = Поле.Значение;
					
				ИначеЕсли Найти(Поле.Ключ, "T1R"+Ноль+НомерБезПробелов+"G03") = 1 Тогда
					СуществуетСтрока = Истина;
					rsMain.Fields.Item("TAB1_A3").Value = Поле.Значение;
					
				ИначеЕсли Найти(Поле.Ключ, "T1R"+Ноль+НомерБезПробелов+"G04A") = 1 Тогда
					СуществуетСтрока = Истина;
					rsMain.Fields.Item("TAB1_A41").Value = Поле.Значение;
					
				ИначеЕсли Найти(Поле.Ключ, "T1R"+Ноль+НомерБезПробелов+"G04") = 1 Тогда
					СуществуетСтрока = Истина;
					rsMain.Fields.Item("TAB1_A4").Value = Поле.Значение;
					
				ИначеЕсли Найти(Поле.Ключ, "T1R"+Ноль+НомерБезПробелов+"G05") = 1 Тогда
					СуществуетСтрока = Истина;
					rsMain.Fields.Item("TAB1_A5").Value = Поле.Значение;
					
				ИначеЕсли Найти(Поле.Ключ, "T1R"+Ноль+НомерБезПробелов+"G06D") = 1 Тогда
				...
				ИначеЕсли Найти(Поле.Ключ, "T1R"+Ноль+НомерБезПробелов+"G07D") = 1 Тогда
				...
				ИначеЕсли Найти(Поле.Ключ, "T1R"+Ноль+НомерБезПробелов+"G08") = 1 Тогда
					СуществуетСтрока = Истина;
					rsMain.Fields.Item("TAB1_A8").Value = ?(Поле.Значение <> "",Число(Поле.Значение),0);
					rsMain.Fields.Item("TAB1_A81").Value = Поле.Значение;
								
				ИначеЕсли Найти(Поле.Ключ, "T1R"+Ноль+НомерБезПробелов+"G09") = 1 Тогда
					СуществуетСтрока = Истина;
...
Процедура Формирование_ЕСВ_ТАБ1(Doc, Бланк, Документ)
	...
	Если Бланк = "J3000412" ИЛИ Бланк = "F3000412" Тогда
		...
		с.Вставить("FIRM_OPFNM",   "HKOPFG");
		с.Вставить("FIRM_PENS",    "HNREG");
		с.Вставить("FIRM_NMBANK",  "HBANKNAME");
		с.Вставить("FIRM_CBANK",   "HMFO");
		с.Вставить("FIRM_RS",      "HBANKACC");
...
Функция Формирование_Декларация_Прибыль(App,  Документ,  Бланк,  PDate,  ИД)
	...
	КонецЕсли;
		//Додаток ПП
	Если Приложения_таб10 = Истина Тогда
		Doc  = App.OpenOrCreateByExDocID(БланкПП, ОпределитьКодПериода(Документ), PDate, , ИД+"-0", 0);
		Doc.IDParent = ИДПарент;	
	Попытка
			Формирование_Декларация_Прибыль_ПП(Doc, БланкПП, Документ);
	Исключение
			Сообщить("Ошибка: Додаток ПП не создан");
			Doc  = App.MoveToTrash(ИД+"-0");
		КонецПопытки;
	Иначе	
		Doc  = App.MoveToTrash(ИД+"-0");
	КонецЕсли;
...
Процедура Формирование_Декларация_Прибыль_ПП(Doc, БланкПП, Документ)
	...
	Если БланкПП="J0112018" Тогда
		Док = Документ.ДанныеОтчета.Получить().ПоказателиОтчета.ПолеТабличногоДокументаПриложениеПП;	
		rsMain = Doc.DataSets();
		с = Новый Структура;
		НачалоОтчета(rsMain);
		...
		КонецЕсли;
		Если Док.HYP <> "" Тогда
			rsMain.Fields.Item("HYP").value = true;
		КонецЕсли;
		
		с.Вставить("REP_NYEAR", "HZY");
		с.Вставить("NYEARP", "HZYP");
	...
	СуществуетСтрока = Ложь;   
		...
		Для Каждого Поле Из Док Цикл
			...
			ИначеЕсли Найти(Поле.Ключ, "T1R"+Ноль+НомерСтроки+"G3") = 1 Тогда
				 СуществуетСтрока = Истина;
				 rsMain.Fields.Item("TAB1_A3").Value = Поле.Значение;	 
			ИначеЕсли Найти(Поле.Ключ, "T1R"+Ноль+НомерСтроки+"G4D") = 1 Тогда
			 Если Поле.Значение <> Дата(1,1,1) Тогда
				 СуществуетСтрока = Истина;
				 rsMain.Fields.Item("TAB1_A4").Value = Поле.Значение;
		     КонецЕсли;
...
Функция Формирование_Декларация_Прибыль_ПН(Doc, Документ, App,  PDate,  ИД, ИДПарент, БланкПН)
	...
	Если БланкПН = "J0111518" Тогда
		...
		Для Каждого Поле Из Док Цикл	
			...
			rsMain.Fields.Item("A13_3").Value = Поле.Данные.R0112G3;
			rsMain.Fields.Item("A14_3").Value = Поле.Данные.R0113G3;
			rsMain.Fields.Item("A15_3").Value = Поле.Данные.R0114G3;
			rsMain.Fields.Item("A16_3").Value = Поле.Данные.R0115G3;
			rsMain.Fields.Item("A17_3").Value = Поле.Данные.R0116G3;
			rsMain.Fields.Item("A18_3").Value = Поле.Данные.R0117G3;
			rsMain.Fields.Item("A19_3").Value = Поле.Данные.R0118G3;
			rsMain.Fields.Item("A20_3").Value = Поле.Данные.R0119G3;
			rsMain.Fields.Item("A21_3").Value = Поле.Данные.R0120G3;
			rsMain.Fields.Item("A22_3").Value = Поле.Данные.R0121G3;
			rsMain.Fields.Item("A23_3").Value = Поле.Данные.R0122G3;
			rsMain.Fields.Item("A24_3").Value = Поле.Данные.R0123G3;
			rsMain.Fields.Item("A25_3").Value = Поле.Данные.R0124G3;
			rsMain.Fields.Item("A26_3").Value = Поле.Данные.R0125G3;
			rsMain.Fields.Item("A27_3").Value = Поле.Данные.R0126G3;
			rsMain.Fields.Item("A0_3").Value = Поле.Данные.R0127G3;
			...
			rsMain.Fields.Item("A13_5").Value = Поле.Данные.R0112G5;
			rsMain.Fields.Item("A14_5").Value = Поле.Данные.R0113G5;
			rsMain.Fields.Item("A15_5").Value = Поле.Данные.R0114G5;
			rsMain.Fields.Item("A16_5").Value = Поле.Данные.R0115G5;
			rsMain.Fields.Item("A17_5").Value = Поле.Данные.R0116G5;
			rsMain.Fields.Item("A18_5").Value = Поле.Данные.R0117G5;
			rsMain.Fields.Item("A19_5").Value = Поле.Данные.R0118G5;
			rsMain.Fields.Item("A20_5").Value = Поле.Данные.R0119G5;
			rsMain.Fields.Item("A21_5").Value = Поле.Данные.R0120G5;
			rsMain.Fields.Item("A24_5").Value = Поле.Данные.R0123G5;
			...
			rsMain.Fields.Item("A13_6").Value = Поле.Данные.R0112G6;
			rsMain.Fields.Item("A14_6").Value = Поле.Данные.R0113G6;
			rsMain.Fields.Item("A15_6").Value = Поле.Данные.R0114G6;
			rsMain.Fields.Item("A16_6").Value = Поле.Данные.R0115G6;
			rsMain.Fields.Item("A17_6").Value = Поле.Данные.R0116G6;
			rsMain.Fields.Item("A18_6").Value = Поле.Данные.R0117G6;
			rsMain.Fields.Item("A19_6").Value = Поле.Данные.R0118G6;
			rsMain.Fields.Item("A20_6").Value = Поле.Данные.R0119G6;
			rsMain.Fields.Item("A21_6").Value = Поле.Данные.R0120G6;
			rsMain.Fields.Item("A24_6").Value = Поле.Данные.R0123G6;
			rsMain.Fields.Item("A0_6").Value = Поле.Данные.R0127G6;
			...
			Если Поле.Данные.H01G3 <> "" Тогда
				rsMain.Fields.Item("N7").Value = Истина; //Поле.Данные.H01G3;
			КонецЕсли; 
...
Функция Формирование_Декларация_НДС(App,  Документ,  Бланк,  PDate,  ИД)
	...
	Если Бланк = "J0200121" Тогда
		...
		БланкДод8 = "J0215821";
		БланкДод9 = "J0215321";
	ИначеЕсли Бланк = "F0200121" Тогда
		БланкДод1 = "F0200421";
		БланкДод2 = "F0215221";
		БланкДод3 = "F0200521";
		БланкДод4 = "F0299821";
	...
	Если ИДПарент <> 0 Тогда
		//Додаток 1
		Если Приложения_таб1 = Истина Тогда
			Doc  = App.OpenOrCreateByExDocID(БланкДод1, ОпределитьКвМес(Документ), PDate, , ИД+"-1", 0);
			Doc.IDParent = ИДПарент;	
			Попытка
		...
		Если Приложения_таб2 = Истина Тогда
			ИД  =  Строка(Строка(Документ.УникальныйИдентификатор()));
			Doc  = App.OpenOrCreateByExDocID(БланкДод2, ОпределитьКвМес(Документ), PDate, , ИД+"-2", 0);
			Doc.IDParent = ИДПарент;	
			Попытка
		...
		//		//Додаток 3
		Если Приложения_таб3 = Истина Тогда
			Doc  = App.OpenOrCreateByExDocID(БланкДод3, ОпределитьКвМес(Документ), PDate, , ИД+"-3", 0);
			Doc.IDParent = ИДПарент;	
			Попытка
		...
		//		//Додаток 4
		Если Приложения_таб4 = Истина Тогда
			Doc  = App.OpenOrCreateByExDocID(БланкДод4, ОпределитьКвМес(Документ), PDate, , ИД+"-4", 0);
			Doc.IDParent = ИДПарент;	
			Попытка
		...
		//		//Додаток 5
		Если Приложения_таб5 = Истина Тогда
			Doc  = App.OpenOrCreateByExDocID(БланкДод5, ОпределитьКвМес(Документ), PDate, , ИД+"-5", 0);
			Doc.IDParent = ИДПарент;	
			Попытка
		...
		//		//Додаток 6
		Если Приложения_таб6 = Истина Тогда
			Doc  = App.OpenOrCreateByExDocID(БланкДод6, ОпределитьКвМес(Документ), PDate, , ИД+"-6", 0);
			Doc.IDParent = ИДПарент;	
			Попытка
		...
		//		//Додаток 7
		Если Приложения_таб7 = Истина Тогда
			Doc  = App.OpenOrCreateByExDocID(БланкДод7, ОпределитьКвМес(Документ), PDate, , ИД+"-7", 0);
			Doc.IDParent = ИДПарент;	
			Попытка
		...
		//		//Додаток 8
		Если Приложения_таб8 = Истина Тогда
			Doc  = App.OpenOrCreateByExDocID(БланкДод8, ОпределитьКвМес(Документ), PDate, , ИД+"-8", 0);
			Doc.IDParent = ИДПарент;	
			Попытка
		...
		//		//Додаток 9
		Если Приложения_таб9 = Истина Тогда
			Doc  = App.OpenOrCreateByExDocID(БланкДод9, ОпределитьКвМес(Документ), PDate, , ИД+"-9", 0);
			Doc.IDParent = ИДПарент;	
			Попытка
...
КонецФункции
//============
Функция Формирование_УточняющаяДекларация_НДС(App,  Документ,  Бланк,  PDate,  ИД)
	
	ИДПарент = 0;
	
	Если Бланк = "J0217021" Тогда
		БланкДод1 = "J0200521";
		БланкДод2 = "J0215221";
		БланкДод3 = "J0200621";
		БланкДод4 = "J0299821";
		...
		БланкДод7 = "J0215721";
		БланкДод9 = "J0215321";
	ИначеЕсли Бланк = "F0217021" Тогда
		БланкДод1 = "F0200421";
		БланкДод2 = "F0215221";
		БланкДод3 = "F0200521";
		БланкДод4 = "F0299821";
...
Функция Формирование_УточняющаяДекларация_НДС_Главный(Doc, Бланк, Документ)
	...
	ИДПарент = 0;
	Если Бланк = "J0217021" ИЛИ Бланк = "F0217021" Тогда
		Док = Документ.ДанныеОтчета.Получить().ПоказателиОтчета.ПолеТабличногоДокументаДекларацияПоНДС;
		rsMain = Doc.DataSets(); 
		НачалоОтчета(rsMain);	
		с = Новый Структура;
...
Процедура Формирование_Декларация_НДС_Дод1(Doc, БланкДод1, Документ)
	...
	Если БланкДод1="J0200521" ИЛИ БланкДод1 = "F0200421" Тогда
			...
			Для Каждого Поле Из Док Цикл
				 ...
				 КонецЕсли;
					...
					ИначеЕсли Найти(Поле.Ключ, "T12"+Ноль+НомерСтроки4+"RG5") = 1 Тогда
						Если Поле.Значение <> "" Тогда
						  rsTab9.Fields.Item("TAB9_CX").Value = Поле.Значение;
						  СуществуетСтрока = Истина;
						КонецЕсли;
...
Процедура Формирование_Декларация_НДС_Дод2(Doc, БланкДод2, Документ)
	...
	Если БланкДод2 = "J0215221" ИЛИ БланкДод2 = "F0215221" Тогда
...
Процедура Формирование_Декларация_НДС_Дод3(Doc, БланкДод3, Документ)
	...
	Если БланкДод3 = "J0200621" ИЛИ БланкДод3 = "F0200521" Тогда
...
Процедура Формирование_Декларация_НДС_Дод4(Doc, БланкДод4, Документ)
	...
	Если БланкДод4="J0299821" ИЛИ БланкДод4 = "F0299821" Тогда
			...
			Для Каждого Поле Из Док Цикл
				 ...
				 Если  Найти(Поле.Ключ, "T6R"+Ноль+НомерСтроки3+"G2") = 1 Тогда
					...
					ИначеЕсли Найти(Поле.Ключ, "T6R"+Ноль+НомерСтроки3+"G3") = 1 Тогда
						  rsTab3.Fields.Item("TAB3_A3").Value = Поле.Значение;
						  СуществуетСтрока = Истина;
					ИначеЕсли Найти(Поле.Ключ, "T6R"+Ноль+НомерСтроки3+"G4") = 1 Тогда
...
Процедура Формирование_Декларация_НДС_Дод5(Doc, БланкДод5, Документ)
	...
	Если БланкДод5 = "J0215121" ИЛИ БланкДод5 = "F0215121" Тогда
		...
		Для Каждого Поле Из ТЧ Цикл   //Док
			...
			Если Поле.Ключ = "П5T1R" Тогда
				...
				Если ЗначениеЗаполнено(Стр.T1RG2) Тогда
					rsTab.Fields.Item("TAB1_A2").Value = Стр.T1RG2;
				КонецЕсли;
				Если ЗначениеЗаполнено(Стр.T1RG3A) Тогда
					rsTab.Fields.Item("TAB1_A5").Value = Стр.T1RG3A;
				КонецЕсли;
				Если ЗначениеЗаполнено(Стр.T1RG3B) Тогда
				...
				КонецЕсли;	
				Если ЗначениеЗаполнено(Стр.T1RG5) Тогда	
					rsTab.Fields.Item("TAB1_A4").Value = Стр.T1RG5;
				КонецЕсли;	
				Если ЗначениеЗаполнено(Стр.T1RG6) Тогда	
					rsTab.Fields.Item("TAB1_A6").Value = Стр.T1RG6;
				КонецЕсли;	
				Если ЗначениеЗаполнено(Стр.T1RG42) Тогда	
			...
			Если Поле.Ключ = "П5T2R" Тогда
				...
				Если ЗначениеЗаполнено(Стр.T2RG2) Тогда	
					rsTab2.Fields.Item("TAB2_A2").Value = Стр.T2RG2;
				КонецЕсли;
				Если ЗначениеЗаполнено(Стр.T2RG3A) Тогда	
					rsTab2.Fields.Item("TAB2_A3").Value = Стр.T2RG3A;
				КонецЕсли;
				Если ЗначениеЗаполнено(Стр.T2RG3B) Тогда
					rsTab2.Fields.Item("TAB2_B3").Value = Стр.T2RG3B;
				КонецЕсли;
				Если Стр.T2RG41 <> "" Тогда
					rsTab2.Fields.Item("TAB2_K4").Value = true;
				КонецЕсли;
				Если ЗначениеЗаполнено(Стр.T2RG4) Тогда
					rsTab2.Fields.Item("TAB2_A4").Value = Стр.T2RG4;
				КонецЕсли;
				Если ЗначениеЗаполнено(Стр.T2RG5) Тогда
					rsTab2.Fields.Item("TAB2_A5").Value = Стр.T2RG5;
				КонецЕсли;
				Если ЗначениеЗаполнено(Стр.T2RG6) Тогда
					rsTab2.Fields.Item("TAB2_A6").Value = Стр.T2RG6;
				КонецЕсли;
				Если ЗначениеЗаполнено(Стр.T2RG42) Тогда
			...
			Если  Поле.Ключ = "П5T12R" Тогда
				...
				Если ЗначениеЗаполнено(Стр.T12RG2) Тогда	
				...
				КонецЕсли;
				Если ЗначениеЗаполнено(Стр.T12RG41) Тогда
					rsTab10.Fields.Item("TAB10_A41").Value = Стр.T12RG41;
				КонецЕсли;
				Если ЗначениеЗаполнено(Стр.T12RG42) Тогда
					rsTab10.Fields.Item("TAB10_A42").Value = Стр.T12RG42;
				КонецЕсли;
				Если ЗначениеЗаполнено(Стр.T12RG43) Тогда
					rsTab10.Fields.Item("TAB10_A43").Value = Стр.T12RG43;
				КонецЕсли;
				Если ЗначениеЗаполнено(Стр.T12RG5) Тогда
					rsTab10.Fields.Item("TAB10_CX").Value = Стр.T12RG5;
				КонецЕсли;
				Если ЗначениеЗаполнено(Стр.T12RG6) Тогда
					rsTab10.Fields.Item("TAB10_A6").Value = Стр.T12RG6;
				КонецЕсли;
				Если ЗначениеЗаполнено(Стр.T12RG7) Тогда
					rsTab10.Fields.Item("TAB10_A7").Value = Стр.T12RG7;
				КонецЕсли;
				Если ЗначениеЗаполнено(Стр.T12RG8) Тогда
...
Процедура Формирование_Декларация_НДС_Дод6(Doc, БланкДод6, Документ)
	...
	Если БланкДод6="J0299321" ИЛИ БланкДод6 = "F0299321" Тогда
		...
		Пока ДобавитьНовуюСтроку Цикл			
			...
			Для Каждого Поле Из Док Цикл
	    		 ...
	    		 КонецЕсли;
				    ...
				    Если  Найти(Поле.Ключ, "T11"+Ноль+НомерСтроки+"RG2S") = 1 Тогда
	    				  rsTab.Fields.Item("TAB1_A2").Value = Поле.Значение;
	    				  СуществуетСтрока = Истина;
	    			ИначеЕсли Найти(Поле.Ключ, "T11"+Ноль+НомерСтроки+"RG3S") = 1 Тогда
	    				  rsTab.Fields.Item("TAB1_A3").Value = Поле.Значение;
	    				  СуществуетСтрока = Истина;	  
	    			ИначеЕсли Найти(Поле.Ключ, "T11"+Ноль+НомерСтроки+"RG4") = 1 Тогда
	    				  rsTab.Fields.Item("TAB1_A4").Value = Поле.Значение;
	    				  СуществуетСтрока = Истина;
	    			КонецЕсли;
		...
		Пока ДобавитьНовуюСтроку Цикл			
			...
			Для Каждого Поле Из Док Цикл
				 ...
				 КонецЕсли;
				    ...
				    Если  Найти(Поле.Ключ, "T2R"+Ноль+НомерСтроки2+"G2S") = 1 Тогда
						  rsTab2.Fields.Item("TAB2_A2").Value = Поле.Значение;
						  СуществуетСтрока = Истина;
					ИначеЕсли Найти(Поле.Ключ, "T2R"+Ноль+НомерСтроки2+"G3") = 1 Тогда
						  rsTab2.Fields.Item("TAB2_A3").Value = Поле.Значение;
				    ...
				    ИначеЕсли Найти(Поле.Ключ, "T2R"+Ноль+НомерСтроки2+"G6") = 1 Тогда
						  rsTab2.Fields.Item("TAB2_A6").Value = Поле.Значение;
						  СуществуетСтрока = Истина;
				    ИначеЕсли Найти(Поле.Ключ, "T2R"+Ноль+НомерСтроки2+"G7") = 1 Тогда
						  rsTab2.Fields.Item("TAB2_A7").Value = Поле.Значение;
						  СуществуетСтрока = Истина;
				    ИначеЕсли Найти(Поле.Ключ, "T2R"+Ноль+НомерСтроки2+"G8") = 1 Тогда
		...
		Пока ДобавитьНовуюСтроку Цикл			
			...
			Для Каждого Поле Из Док Цикл
				 ...
				 КонецЕсли;
				    ...
				    Если  Найти(Поле.Ключ, "T12"+Ноль+НомерСтроки3+"RG2S") = 1 Тогда
						  rsTab3.Fields.Item("TAB3_A2").Value = Поле.Значение;
						  СуществуетСтрока = Истина;
					ИначеЕсли Найти(Поле.Ключ, "T12"+Ноль+НомерСтроки3+"RG3S") = 1 Тогда
						  rsTab3.Fields.Item("TAB3_A3").Value = Поле.Значение;
						  СуществуетСтрока = Истина;
					ИначеЕсли Найти(Поле.Ключ, "T12"+Ноль+НомерСтроки3+"RG4") = 1 Тогда
						  rsTab3.Fields.Item("TAB3_A4").Value = Поле.Значение;
						  СуществуетСтрока = Истина;
					КонецЕсли;
...
Процедура Формирование_Декларация_НДС_Дод7(Doc, БланкДод7, Документ)
	...
	Если БланкДод7="J0215721" ИЛИ БланкДод7 = "F0215721" Тогда
...
Процедура Формирование_Декларация_НДС_Дод8(Doc, БланкДод8, Документ)
	...
	Если БланкДод8="J0215821" ИЛИ БланкДод8 = "F0215821" Тогда
...
Процедура НомДобавить(НоваяСтрока,  НоменклатураДока,  НомКоличество,  НомЦена,  НомСумма,  НомСтавкаНДС, СуммаНДС, ЕдИзмНоменклатуры)
	...
	Попытка
		...
		НоваяСтрока.ЕдиницаИзмерения = НоменклатураДока.ЕдиницаДляОтчетов;		
...
Процедура СозданиеНоменклатуры(ДатаДокаВх,  НомерДокаВх,  НоменклатураОригинал,  НаОсновании,  ЕИОригинал,  НомСтавкаНДС, ВидНоменклатуры)	
	...
	//Если СоздаватьНоменклатуру Тогда
	    ...
	    ЕдИзмНоменклатура = Справочники.ЕдиницыИзмерения.СоздатьЭлемент();  
	    ЕдИзмНоменклатура.Владелец =  НоменклатураМедка.Ссылка;                  
	    ЕдИзмНоменклатура.Наименование =  ЕИОригинал;                  
		ЕдИзмНоменклатура.Коэффициент = 1;                           
	    ЕдИзмКлассификатор  = Справочники.КлассификаторЕдиницИзмерения.НайтиПоНаименованию(ЕИОригинал);   
	    ЕдИзмНоменклатура.ЕдиницаПоКлассификатору = ЕдИзмКлассификатор;   
...
Процедура ОрганизацияПриИзменении(Элемент)
	...
	КонецЕсли;
	   ...
	   // Блок для настройки отображения только для указанных кодов
  	    ОбновлениеОтображенияКнопокПервичка();
		СписокПервички.Очистить();
...
Процедура ЗаполнитьРеквизитыДокументовРеализацияУслугПоПереработке(СтрокиТаблицыДокументов  =  Неопределено,  ПроверитьСостояниеРегистрации  =  Истина,  УстановитьПометки  =  Истина)
	...
	|	ГДЕ
	|		РеализацияУслугПоПереработке.Ссылка В(&МассивДокументов)) КАК ВложенныйЗапрос";
...
Процедура ВыбратьПервичку()
	...
	Если ТипДок = "РеализацияУслугПоПереработке" Тогда
		...
		|		Документ.РеализацияУслугПоПереработке КАК РеализацияУслугПоПереработке
		|	ГДЕ
		|		РеализацияУслугПоПереработке.Дата МЕЖДУ &НачПериода И &КонПериода
		|		И РеализацияУслугПоПереработке.Организация  =  &Организация
		|		И РеализацияУслугПоПереработке.Продукция.Количество>0) КАК ВложенныйЗапрос
		|{ГДЕ
		|	ВложенныйЗапрос.Контрагент, 
		|	ВложенныйЗапрос.Ответственный, 
		...
		Если ТаблицаДокументов.Количество() > 0 Тогда
			...
			ЗаполнитьРеквизитыДокументовРеализацияУслугПоПереработке(); 
		Иначе 
			ИзменениеОсновныхПараметров();
		КонецЕсли;
		...
		Скрытие(БезНДСЧисло, ВсегоНДСЧисло, ВсегоЧисло1, ВыбраноБезНДСЧисло, ВыбраноВсегоНДСЧисло, ВыбраноВсегоЧисло);
		...
		Для каждого ПодобранныйДокумент Из ПодобранныеДокументы Цикл
			...
			Если НЕ ИмяБазы = "УТ" Тогда
				...
				Иначе
				    ...
				    Иначе
						НоваяСтрока.СуммаДокумента  =  Док.Услуги.Итог("Сумма") + Док.Услуги.Итог("СуммаНДС"); 
						НоваяСтрока.СуммаНДСДокумента = Док.Услуги.Итог("СуммаНДС");
						НоваяСтрока.СуммаБезНДС  =  Док.Услуги.Итог("Сумма"); 					
					КонецЕсли;
				КонецЕсли;
			Иначе
				НоваяСтрока.СуммаДокумента  =  Док.Услуги.Итог("Сумма"); 
...
// Серна - Начало
				...
				Если (Лев(ПодобранныйДокумент.Документ.Комментарий, 5) = "MEDOC") или (Лев(ПодобранныйДокумент.Документ.Комментарий, 5) = "Переданий з M.E.Doc") Тогда
// Серна - Конец
				Иначе 
					Продолжить;
				КонецЕсли; 
...
Процедура ПроверитьНаличиеПервички()
	...
	Попытка 
		...
		Для Каждого СтрокаДокумент из ТаблицаДокументов Цикл
			...
			КонецЕсли;     		
// Серна - Начало
			Если (СтрокаДокумент.Статус="Принято,  Правильный") или (СтрокаДокумент.Статус="Получено сообщение,  Правильный")
				или (СтрокаДокумент.Статус="Импортирован,  Правильный") Тогда
				Если (СтрокаДокумент.ВключенаВЕдиныйРеестрНалоговыхНакладных = 0) Тогда
					ТекущийДокумент = СтрокаДокумент.Документ.Ссылка.ПолучитьОбъект();
					ТекущийДокумент.ВключенаВЕдиныйРеестрНалоговыхНакладных = Истина;
...
Процедура ПриОткрытии() 
	...
	ЭлементыФормы.НЕПередаватьРеквизитыДоговора.Значение       	 =  ВосстановитьЗначение("НЕПередаватьРеквизитыДоговора");
	ЭлементыФормы.НЕСоздаватьНовыйДоговор.Значение       	 =  ВосстановитьЗначение("НЕСоздаватьНовыйДоговор");
	ЭлементыФормы.НЕЗаполнятьКомментарии.Значение       	 =  ВосстановитьЗначение("НЕЗаполнятьКомментарии");	
...
// Серна - Начало
	...
	ЭлементыФормы.Вкл.Значение  =  1;
// Серна - Конец
	
	ВыборЯрлыка();
	
	Если ИмяБазы = "УПП" Тогда //ИЛИ ИмяБазы = "УТП"
		СписокРеглОтчетов.Добавить("сепаратор",  Нстр("ru = '-------------------------------------------';uk = '-------------------------------------------'"));
		СписокРеглОтчетов.Добавить("ЗаявлениеРасчетФСС",  Нстр("ru = 'Заявление-расчет в ФСС';uk = 'Заява-розрахунок в ФСС'"));  
	КонецЕсли;
	...
	КонецЕСли;
// Серна - Начало
	// Добавим дату запрета редактирования
	Если ПолучитьИмяБазы() = "БУ" Тогда
		Запрос = Новый Запрос;
		Запрос.Текст =
		"ВЫБРАТЬ ГраницыЗапретаИзмененияДанных.ГраницаЗапретаИзмененийНН
		|ИЗ РегистрСведений.ГраницыЗапретаИзмененияДанных КАК ГраницыЗапретаИзмененияДанных
		|ГДЕ ГраницыЗапретаИзмененияДанных.Пользователь = НЕОПРЕДЕЛЕНО
		|";
		Выборка = Запрос.Выполнить().Выбрать();
		Если Выборка.Следующий() Тогда
			ДатаЗапрета = Выборка.ГраницаЗапретаИзмененийНН;
		КонецЕсли;
	ИначеЕсли ПолучитьИмяБазы() = "ЗУП" Тогда
		Запрос = Новый Запрос;
		Запрос.Текст =
		"ВЫБРАТЬ МАКСИМУМ(Рег.ГраницаЗапретаИзменений) КАК ГраницаЗапретаИзменений ИЗ РегистрСведений.ГраницыЗапретаИзмененияДанных КАК Рег";
		Выборка = Запрос.Выполнить().Выбрать();
		Если Выборка.Следующий() Тогда
			ДатаЗапрета = Выборка.ГраницаЗапретаИзменений;
		КонецЕсли;
	КонецЕсли;
	
	ЭлементыФормы.ПерГраницыЗапретаИзмененияДанных.Значение=ДатаЗапрета;
// Серна - Конец
...
Процедура ПриЗакрытии()
	...
	СохранитьЗначение("НЕПередаватьРеквизитыДоговора",       ЭлементыФормы.НЕПередаватьРеквизитыДоговора.Значение);
	СохранитьЗначение("НЕСоздаватьНовыйДоговор",       ЭлементыФормы.НЕСоздаватьНовыйДоговор.Значение);	
	СохранитьЗначение("НЕЗаполнятьКомментарии",       ЭлементыФормы.НЕЗаполнятьКомментарии.Значение);
...
Процедура ОбновлениеОтображенияКнопокПервичка()
	...
	Если СведенияОПоставщике.КодПоЕДРПОУ = "39404434" ИЛИ СведенияОПоставщике.КодПоЕДРПОУ = "39058283" ИЛИ СведенияОПоставщике.КодПоЕДРПОУ = "32912338" Тогда
		ЭлементыФормы.КоманднаяПанельТаблицаДокументов.Кнопки.ПередатьСвойШаблон.Отображение = ОтображениеКнопкиКоманднойПанели.НадписьКартинка;
		ЭлементыФормы.КоманднаяПанельТаблицаДокументов.Кнопки.ПередатьСвойШаблон.Текст = "Передать в M.E.Doc (свой шаблон)";
		ЭлементыФормы.КоманднаяПанельТаблицаДокументов.Кнопки.ПередатьСвойШаблон.Доступность = Истина;
	Иначе
		ЭлементыФормы.КоманднаяПанельТаблицаДокументов.Кнопки.ПередатьСвойШаблон.Отображение = ОтображениеКнопкиКоманднойПанели.Надпись;
		ЭлементыФормы.КоманднаяПанельТаблицаДокументов.Кнопки.ПередатьСвойШаблон.Текст = "";
		ЭлементыФормы.КоманднаяПанельТаблицаДокументов.Кнопки.ПередатьСвойШаблон.Доступность = Ложь;
	КонецЕсли; 	 
...
Процедура ОтобразитьВозможности()
	...
	Если ТипДок = "ОказаниеУслуг" или ТипДок = "Счет" или ТипДок = "АктСверки" или ТипДок = "ВидНакладна" или ТипДок = "ВозвратТоваровОтПокупателя"
		или ТипДок = "ВозвратТоваровПоставщику" или ТипДок = "РеализацияУслугПоПереработке" Тогда
		ЭлементыФормы.х2.Доступность = Ложь;
...
Процедура КоманднаяПанель1ЗаполнитьДокументами(Кнопка)
	...
	Если ИмяБазы = "УПП" ИЛИ ИмяБазы = "УТП" 
		И ТипДокОтчета = "ЗаявлениеРасчетФСС" Тогда
		
		Запрос.Текст =  
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ЗаявлениеРасчетВФСС.Ссылка КАК ДокументОтчет,
		|	ЗаявлениеРасчетВФСС.ПометкаУдаления КАК ПометкаУдаленияОтчета,
		|	ЗаявлениеРасчетВФСС.ПериодРегистрации,
		|	ЗаявлениеРасчетВФСС.Дата КАК Модифицирован
		|ИЗ
		|	Документ.ЗаявлениеРасчетВФСС КАК ЗаявлениеРасчетВФСС
		|ГДЕ
		|	ЗаявлениеРасчетВФСС.Дата МЕЖДУ &НачПериодаОтч И &КонПериодаОтч
		|	И ЗаявлениеРасчетВФСС.Организация = &ОрганизацияОтчета
		|
		|УПОРЯДОЧИТЬ ПО
		|	ЗаявлениеРасчетВФСС.Дата УБЫВ";
		...
		Пока Выборка.Следующий() Цикл
			НовыйРядок = ТаблицаОтчетов.Добавить();	
			ЗаполнитьЗначенияСвойств(НовыйРядок,  Выборка);
			НовыйРядок.Название = Нстр("ru = 'Заявление-расчет в ФСС';uk = 'Заява-розрахунок в ФСС'");
			НовыйРядок.ПериодОтчета = РаботаСДиалогами.ДатаКакМесяцПредставление(Выборка.ПериодРегистрации);
		КонецЦикла;
	КонецЕсли;
...
Процедура КоманднаяПанель1ДействиеОтчеты(Кнопка)
	...
	Попытка
		...
		Если ОргМедок = СокрЛП(КодПоЕДРПОУ1) И ОргМедокФиллиал = СокрЛП(пФил1) Тогда 
			orgId  =  App.SeekOrg(СокрЛП(КодПоЕДРПОУ1), СокрЛП(пФил1));
		Иначе
			orgId = 0;
		КонецЕсли;
	...
	Для каждого ДокументОтчет Из МассивОтчетовДляПередачи Цикл  
		...
		ИначеЕсли ТипДокОтчета = "РегламентированныйОтчетФСС" Тогда 			
			...
			КонецЕсли;
			//------------------------Заявка-расчет--ФСС--------------------------------			
		ИначеЕсли ТипДокОтчета = "ЗаявлениеРасчетФСС" Тогда 			
			БланкЗаявкаФСС = "C1100101";
			ИД = Строка(Строка(ДокументОтчет.УникальныйИдентификатор()));
			Если ОтчФСС Тогда
				PDate = Дата(Год(ДокументОтчет.Дата), Месяц(ДокументОтчет.Дата), 01); 
				Если ПерезаписьОтчетов = "Да" Тогда  
					Если App.GetSendSTTByExDocID(ИД) <> -1 Тогда
						App.MoveToTrash(ИД);
					КонецЕсли;  
					Формирование_ЗаявкаФСС(App, ДокументОтчет, БланкЗаявкаФСС, PDate, ИД);
					КолвоФСС = КолвоФСС+1; 
					СписокДляОткрытияОтчет.Добавить(ИД); 						 
				ИначеЕсли ПерезаписьОтчетов = "Нет" Тогда 
					Если App.GetSendSTTByExDocID(ИД) <> -1 Тогда
		...
		ИначеЕсли ТипДокОтчета = "РегламентированныйОтчетДекларацияНДС" Тогда
			...
			Иначе
				...
				Возврат;
			КонецЕсли
			//-----------------------Уточняющая-Декларация-по-НДС------------------------------- 
		ИначеЕсли ТипДокОтчета = "РегламентированныйОтчетУточняющийРасчетДекларацияНДС" Тогда
			Если Приложения_таб1 = Истина Или Приложения_таб2 = Истина Или Приложения_таб3 = Истина Или Приложения_таб4 = Истина Или Приложения_таб5 = Истина 
				Или Приложения_таб6 = Истина Или Приложения_таб7 = Истина Или Приложения_таб8 = Истина Или Приложения_таб9 = Истина Тогда
				Если ОтчНал Тогда
					Если ПерезаписьОтчетов = "Да" Тогда
						...
						PDate = Дата(Год(ДокументОтчет.Дата), Месяц(ДокументОтчет.Дата), 01);
						ИД  =  Строка(Строка(ДокументОтчет.УникальныйИдентификатор()));
						Формирование_УточняющаяДекларация_НДС(App,  ДокументОтчет,  Бланк,  PDate,  ИД);
						КолвоДНДС = КолвоДНДС+1
					ИначеЕсли ПерезаписьОтчетов = "Нет" Тогда
						Продолжить;
					Иначе
...
Процедура РегОтчетОчиститьЧекбоксы();
	...
	Приложения_таб1 = Ложь;
	Приложения_таб2 = Ложь;
	Приложения_таб3 = Ложь;
	Приложения_таб4 = Ложь;
	Приложения_таб5 = Ложь;
	Приложения_таб6 = Ложь;
	Приложения_таб7 = Ложь;
	Приложения_таб8 = Ложь;
	Приложения_таб9 = Ложь;
	Приложения_таб10 = Ложь;
...
// Серна - Начало
					НомерРегВх=СОКРЛП(Отчет.GetValue("MAIN",  "SEND_DPA_RN"));
					
					СпецРежимДляНомераВх =  СокрЛП(Отчет.GetValue("MAIN",  "N2_12")) ;
					Если  НЕ ЗначениеЗаполнено(СпецРежимДляНомераВх)Тогда
						СпецРежимДляНомераВх = Строка(" ");
					Иначе
						СпецРежимДляНомераВх=  СокрЛП(Отчет.GetValue("MAIN",  "N2_12")) ;
					КонецЕсли;	
					ФилиалВхДок = Строка(Отчет.GetValue("MAIN",  "N2_13"));
					Если ФилиалВхДок = "" Тогда
						НомерДокаВх  =  Строка(Отчет.GetValue("MAIN",  "N2_11")+"/"+СпецРежимДляНомераВх+"/");
					Иначе
						НомерДокаВх  = Строка(Отчет.GetValue("MAIN",  "N2_11") +"/"+СпецРежимДляНомераВх+"/"+ ФилиалВхДок);
					КонецЕсли;
// Серна - Конец
					...
					Если ИмяБазы = "БУ" 
						...
						ВидОперации = ННВх.ВидОперации;
// Серна - Начало
						ННВх.ОбменДанными.Загрузка = Истина;
// Серна - Конец
...
// Серна - Начало
					Если   Дата(Отчет.GetValue("MAIN",  "N11"))<= ПерГраницыЗапретаИзмененияДанных Тогда  //+86400
						ННВх.Дата  = НачалоМесяца(ТекущаяДата()) ;	
					Иначе
						ННВх.Дата  =  Дата(Отчет.GetValue("MAIN",  "N11"));
					КонецЕСли;
					// Меняем дату документа на дату рег. в ЕРНН
					// ННВх.Дата  =  Дата(Отчет.GetValue("MAIN",  "SEND_DPA_DATE"));
					Если  ЗначениеЗаполнено(Отчет.GetValue("MAIN",  "SEND_DPA_DATE"))=Истина Тогда
						ННВх.ДатаВключенияВЕдиныйРеестрНалоговыхНакладных=Дата(Отчет.GetValue("MAIN",  "SEND_DPA_DATE"));
					КонецЕсли;
					СпецРежимДляНомераВх=  СокрЛП(Отчет.GetValue("MAIN",  "N2_12")) ;
					Если  НЕ ЗначениеЗаполнено(СпецРежимДляНомераВх)Тогда
						СпецРежимДляНомераВх=Строка(" ");
					...
					//Добавим НомерРегистрации в ЕРПН
					ННВх.НомерРегистрацииВЕРПН=СОКРЛП(Отчет.GetValue("MAIN",  "SEND_DPA_RN"));
					
					ННВх.ДатаВходящегоДокумента  =  Дата(Отчет.GetValue("MAIN",  "N11"));
					Филиал =  Строка(Отчет.GetValue("MAIN",  "N2_13"));
					Если Филиал = "" Тогда
						ННВх.НомерВходящегоДокумента  =  Строка(Отчет.GetValue("MAIN",  "N2_11")+"/"+СпецРежимДляНомераВх+"/");
					Иначе
						ННВх.НомерВходящегоДокумента  =  Строка(Отчет.GetValue("MAIN",  "N2_11") +"/"+СпецРежимДляНомераВх+"/"+ Филиал);	
					КонецЕсли;
// Серна - Конец
					...
					Если Организация = Справочники.Организации.ПустаяСсылка() или Организация = НЕОПРЕДЕЛЕНО Тогда
...
// Серна - Начало
					// Поиск контрагента по : ЕДРПОУ - ИНН - Наименование
					ЕДРПОУПокупателя = Строка(СокрЛП(Отчет.GetValue("MAIN",  "FIRM_EDRPOU")));
					ИННПокупателя	 = Строка(СокрЛП(Отчет.GetValue("MAIN",  "FIRM_INN")));
					Наименование 	 = Строка(СокрЛП(Отчет.GetValue("MAIN",  "FIRM_NAME")));
					
					Покупатель = Справочники.Контрагенты.ПустаяСсылка();
					Если ЗначениеЗаполнено(ЕДРПОУПокупателя) Тогда
						Покупатель = Справочники.Контрагенты.НайтиПоРеквизиту("КодПоЕДРПОУ",  ЕДРПОУПокупателя);
					КонецЕсли;
					Если ЗначениеЗаполнено(ИННПокупателя) и Покупатель.Пустая() Тогда
						Покупатель  =  Справочники.Контрагенты.НайтиПоРеквизиту("ИНН",  ИННПокупателя);
					КонецЕсли;
					Если ЗначениеЗаполнено(Наименование) и Покупатель.Пустая() Тогда
						Покупатель  =  Справочники.Контрагенты.НайтиПоНаименованию( Наименование, Истина );
					КонецЕсли;
					// Для найденного контрагента проверим заполненность реквизитов КодПоЕДРПОУ и ИНН. При необходимости возьмем его из МЕДок
					Если ЗначениеЗаполнено(Покупатель) Тогда
						Если ЗначениеЗаполнено(ЕДРПОУПокупателя) и Не ЗначениеЗаполнено(Покупатель.КодПоЕДРПОУ) 
							или ЗначениеЗаполнено(ИННПокупателя) и Не ЗначениеЗаполнено(Покупатель.ИНН) Тогда
							лКонтрагентОбъект = Покупатель.ПолучитьОбъект();
							Если ЗначениеЗаполнено(ЕДРПОУПокупателя) и Не ЗначениеЗаполнено(Покупатель.КодПоЕДРПОУ) Тогда
								лКонтрагентОбъект.КодПоЕДРПОУ = ЕДРПОУПокупателя;
							КонецЕсли;
							Если ЗначениеЗаполнено(ИННПокупателя) и Не ЗначениеЗаполнено(Покупатель.ИНН) Тогда
								лКонтрагентОбъект.ИНН = ИННПокупателя;
							КонецЕсли;
							лКонтрагентОбъект.ОбменДанными.Загрузка = Истина;
							лКонтрагентОбъект.Записать();
						КонецЕсли;
						Если ЗначениеЗаполнено(ЕДРПОУПокупателя) и Не СокрЛП(Покупатель.КодПоЕДРПОУ) = ЕДРПОУПокупателя Тогда
							Сообщить("Внимание! Для контрагента - " + Покупатель + " не совпадают коды ЕДРПОУ в карточке контрагента - " + СокрЛП(Покупатель.КодПоЕДРПОУ) + " и в документе из МеДок - " + ЕДРПОУПокупателя);
						КонецЕсли;
						Если ЗначениеЗаполнено(ИННПокупателя) и Не СокрЛП(Покупатель.ИНН) = ИННПокупателя Тогда
							Сообщить("Внимание! Для контрагента - " + Покупатель + " не совпадают коды ИНН в карточке контрагента - " + СокрЛП(Покупатель.ИНН) + " и в документе из МеДок - " + ИННПокупателя);
						КонецЕсли;
					КонецЕсли;
// Серна - Конец
					...
					КонецЕсли;
// Серна - Начало
					Если ПроверитьДокументВ1С(СуществованиеДокаНазвание,  Организация, НомерРегВх, СпецРежимДляНомераВх, ННВх.Дата)	Тогда
...
// Серна - Конец
					...
					Попытка
						Если ЗначениеЗаполнено(Отчет.GetValue("MAIN", "N82")) и ЗначениеЗаполнено(Отчет.GetValue("MAIN",  "N81")) Тогда
							...
							КонецЕсли;
// Серна - Начало
							ННВх.ДоговорМедок = ?((ДатаДог = Неопределено) и (НомерДог = Неопределено), "", НомерДог + " від " + ДатаДог);
							ННВх.ДатаДоговорМедок = ДатаДог;
// Серна - Конец
						Иначе 
					...
					КонецПопытки;
// Серна - Начало
					ННВх.ВидДоговораМедок =СокрЛП(Отчет.GetValue("MAIN",  "N8")) ;
					ННВх.ФормаРасчетовМедок =СокрЛП(Отчет.GetValue("MAIN",  "N9")) ;
					ННВх.ВыписалМедок =СокрЛП(Отчет.GetValue("MAIN",  "N10")) ;
					ННВх.АдрессПродавцаМедок =СокрЛП(Отчет.GetValue("MAIN",  "FIRM_ADR")) ;
					ННВх.ТеллефонПродавцаМедок =СокрЛП(Отчет.GetValue("MAIN",  "PHON")) ;
// Серна - Конец
					//--------------------------------------------------------------
					Попытка
					...
					КонецЕсли; 
// Серна - Начало
					ННВх.СпецРежимНалогообложения1 = СокрЛП(Отчет.GetValue("MAIN",  "N2_12")); 
					ННвх.Комментарий = "MEDOC від " + ТекущаяДата();
					ННвх.Ответственный="";
// Серна - Конец
...
// Серна - Начало
						Новаястрока.ТМЦМедок 		= Отчет.GetValue("TAB1",  "TAB1_A13");
						лКодУКТВЭД = СокрЛП( Отчет.GetValue("TAB1",  "TAB1_A131") ); 		// Код УКТВЭД
						Если ПустаяСтрока(лКодУКТВЭД) Тогда
							лКодУКТВЭД = СокрЛП( Отчет.GetValue("TAB1",  "TAB1_A133") );	// Код Услуги
						ИначеЕсли СтрДлина( лКодУКТВЭД ) = 10 Тогда 
							лКодУКТВЭД = Сред(лКодУКТВЭД, 1, 4) + " " + Сред(лКодУКТВЭД, 5, 2) + " " + Сред(лКодУКТВЭД, 7, 2) + " " + Сред(лКодУКТВЭД, 9, 2);
						КонецЕсли;
						Новаястрока.КодУКТВЭД 		= лКодУКТВЭД;
						Новаястрока.КодЕдИзмМедок  	= Отчет.GetValue("TAB1",  "TAB1_A141");
						Новаястрока.ЕдИзмМедок		= Отчет.GetValue("TAB1",  "TAB1_A14");
						Новаястрока.КвоМедок		= Отчет.GetValue("TAB1",  "TAB1_A15");
						Новаястрока.ЦенаМедок		= Отчет.GetValue("TAB1",  "TAB1_A16");
// Серна - Конец
					...
					Если Отчет.GetValue("TAB1", "TAB1_A8")=20 Тогда
							...
							ИначеЕсли ИмяБазы = "УПП"
								...
								НоваяСтрока.НалоговоеНазначение = ?(Справочники.НалоговыеНазначенияАктивовИЗатрат.НайтиПоНаименованию("Обл. НДС") <>
																	...
																	Справочники.НалоговыеНазначенияАктивовИЗатрат.НайтиПоНаименованию("Обл. НДС"),
																	Справочники.НалоговыеНазначенияАктивовИЗатрат.НайтиПоНаименованию("Опод. ПДВ"));
							КонецЕсли;
						//ставка 7%
						ИначеЕсли  Отчет.GetValue("TAB1", "TAB1_A8")=7 Тогда
							...
							КонецЕсли;
						//ставка 14%
						ИначеЕсли  Отчет.GetValue("TAB1", "TAB1_A8")=14 Тогда
...
// Серна - Начало
					// В связи с записью дока в режиме Обмен.Загрузка = Истина, заполним обязательные реквизиты шапки
					ННВх.СуммаДокумента 	  = ННВх.Товары.Итог("СуммаВзаиморасчетов");
					ННВх.СуммаНДСДокумента = ННВх.Товары.Итог("СуммаНДС");
					Если ННВх.СчетНДСУсловнаяПродажа.Пустая() Тогда
						ННВх.СчетНДСУсловнаяПродажа = ПланыСчетов.Хозрасчетный.КорректировкиНалоговыхОбязательств;
					КонецЕсли;
// Серна - Конец
					ННВх.Записать(); 
					//---------------------------------------------------------------------------
...
// Серна - Начало
					...
					СпецРежимВхДод2=СокрЛП(Отчет.GetValue("MAIN",  "N1_12"));
					Если НЕ ЗначениеЗаполнено(СпецРежимВхДод2) Тогда
						СпецРежимВхДод2=Строка(" ");
					Иначе
						СпецРежимВхДод2=СокрЛП(Отчет.GetValue("MAIN",  "N1_12"));
					КонецЕсли;
					
					Если ФилиалВхДок = "" Тогда
						НомерДокаВх  = Строка(Отчет.GetValue("MAIN",  "N1_11")+"/"+СпецРежимВхДод2+"/");
					Иначе
						НомерДокаВх  = Строка(Отчет.GetValue("MAIN",  "N1_11")+"/"+СпецРежимВхДод2+ "/" + Филиал);
					КонецЕсли;  					
// Серна - Конец
...
// Серна - Начало
					Если   Дата(Отчет.GetValue("MAIN",  "N15"))<= ПерГраницыЗапретаИзмененияДанных Тогда
						Дод2.Дата  = НачалоМесяца(ТекущаяДата()) ;	
					Иначе
						Дод2.Дата  =  Дата(Отчет.GetValue("MAIN",  "N15"));
					КонецЕСли;
					Дод2.ДатаВключенияВЕдиныйРеестрНалоговыхНакладных=Дата(Отчет.GetValue("MAIN",  "SEND_DPA_DATE"));
// Серна - Конец
...
// Серна - Начало
					...
					Иначе
						СпецРежимВхДод2=СокрЛП(Отчет.GetValue("MAIN",  "N1_12"));
					КонецЕсли;
					// Добавим номер регистрации в ЕРПН
					Дод2.НомерРегистрацииВЕРПН=СОКРЛП(Отчет.GetValue("MAIN",  "SEND_DPA_RN"));
					
					ФилиалПродавца =  Строка(Отчет.GetValue("MAIN",  "N1_13"));
					Если НЕ ЗначениеЗаполнено(ФилиалПродавца) Тогда
						Дод2.НомерВходящегоДокумента  =  Строка(Отчет.GetValue("MAIN",  "N1_11")+"/"+СпецРежимВхДод2+"/");
					Иначе
						Дод2.НомерВходящегоДокумента  =  Строка(Отчет.GetValue("MAIN",  "N1_11")+"/"+СпецРежимВхДод2+ "/" + ФилиалПродавца);	
					КонецЕсли;
// Серна - Конец
					//---------------------------------------------------------------------------
...
// Серна - Начало
					СпецРежимВхДод2Код=СокрЛП(Отчет.GetValue("MAIN",  "N2_12"));
					Если НЕ ЗначениеЗаполнено(Филиал2) Тогда
						Дод2.НомерКорректируемогоВходящегоДокумента  =  Строка(Отчет.GetValue("MAIN",  "N2_11")+"/"+СпецРежимВхДод2Код+"/");
					Иначе
						Дод2.НомерКорректируемогоВходящегоДокумента  =  Строка(Отчет.GetValue("MAIN",  "N2_11")+"/"+СпецРежимВхДод2Код + "/" + Филиал2);	
					КонецЕсли;  
// Серна - Конец
					//---------------------------------------------------------------------------
					ИННОрганизации  =  Строка(СокрЛП(Отчет.GetValue("MAIN",  "N4")));
					...
					Если НЕ ЗначениеЗаполнено(Продавец) Тогда 
						...
						Если СозданиеКонтрагентов Тогда
							Продавец = СозданиеКонтрагента(Отчет, НазваниеДока, НомерДокаВх, ДатаДокаВх, ФилиалВхДок);	
						Иначе
							МедКомпонент.CancelDoc("Дододаток 2 " + СокрЛП(Дод2.НомерВходящегоДокумента) + " від " + Формат(Дод2.ДатаВходящегоДокумента,  "ДЛФ = Д") 
													+ Символы.ПС + "Контрагента з ІПН = " + КонтрИНН + " та ЄДРПОУ = " +КонтрЕДРПОУ+" не існує в 1С!" + Символы.ПС 
													+ "Створення документу неможливо!"  + Символы.ПС + "Увімкніть функцію автоматичного створення контрагента!");
							
							Сообщение = Новый СообщениеПользователю();
							Сообщение.Текст = ("Дододаток 2 " + СокрЛП(Дод2.НомерВходящегоДокумента) + " від " + Формат(Дод2.ДатаВходящегоДокумента,  "ДЛФ = Д") 
													+ Символы.ПС + "Контрагента з ІПН = " + КонтрИНН + " та ЄДРПОУ = " +КонтрЕДРПОУ+" не існує в 1С!" + Символы.ПС 
													+ "Створення документу неможливо!"  + Символы.ПС + "Увімкніть функцію автоматичного створення контрагента!");
							Сообщение.Сообщить();
							Возврат;
                    ...
                    КонецЕсли;	
// Серна - Начало
					//Если ПроверитьСуществованиеДокумента(СуществованиеДокаНазвание, Организация, Продавец, ДатаДокаВх, НомерДокаВх, ВидОперации) Тогда
					Если ПроверитьДокументВ1С(СуществованиеДокаНазвание,  Организация, НомерРегВх, "", Дод2.Дата)	Тогда
// Серна - Конец
						ДокументСуществует(НазваниеДока,  ДатаДокаВх,  НомерДокаВх);
						Возврат;
					КонецЕсли;     
					...
					КонецПопытки;
// Серна - Начало
					// Для печатной формы НН
					Дод2.ДоговорМедок = ?((ДатаДог = Неопределено) и (НомерДог = Неопределено), "", НомерДог + " від " + ДатаДог);
					Дод2.ВидДоговораМедок = СокрЛП(Отчет.GetValue("MAIN",  "N8")) ;
					Дод2.ФормаРасчетовМедок = СокрЛП(Отчет.GetValue("MAIN",  "N9")) ;
					Дод2.ВыписалМедок = СокрЛП(Отчет.GetValue("MAIN",  "N10")) ;
					Дод2.АдрессПродавцаМедок = СокрЛП(Отчет.GetValue("MAIN",  "FIRM_ADR")) ;
					Дод2.ТеллефонПродавцаМедок = СокрЛП(Отчет.GetValue("MAIN",  "PHON")) ;
					Дод2.ДатаДоговорМедок = ДатаДог;
// Серна - Конец
					//------------------------------------------------------------------------------
					...
					КонецЕсли;					
// Серна - Начало
					Дод2.СпецРежимНалогообложения1 = СокрЛП(Отчет.GetValue("MAIN",  "N1_12")); 
					Дод2.Комментарий="MEDOC від "+ТекущаяДата();;
					Дод2.Ответственный="";
// Серна - Конец
					...
					table = Отчет.First("TAB1");
// Серна - Начало
					Дод2.ОснованиеКорректировки=Отчет.GetValue("TAB1",  "TAB1_A2");
// Серна - Конец
					Пока table Цикл
						НоваяСтрока = Дод2.Товары.Добавить();
// Серна - Начало
						НоваяСтрока.ТМЦМедок=Отчет.GetValue("TAB1",  "TAB1_A3");
						лКодУКТВЭД = СокрЛП( Отчет.GetValue("TAB1",  "TAB1_A31") ); 		// Код УКТВЭД
						Если ПустаяСтрока(лКодУКТВЭД) Тогда
							лКодУКТВЭД = СокрЛП( Отчет.GetValue("TAB1",  "TAB1_A33") );		// Код Услуги
						ИначеЕсли СтрДлина( лКодУКТВЭД ) = 10 Тогда 
							лКодУКТВЭД = Сред(лКодУКТВЭД, 1, 4) + " " + Сред(лКодУКТВЭД, 5, 2) + " " + Сред(лКодУКТВЭД, 7, 2) + " " + Сред(лКодУКТВЭД, 9, 2);
						КонецЕсли;
						Новаястрока.КодУКТВЭД 		= лКодУКТВЭД;
						НоваяСтрока.КодЕдИзмМедок=Отчет.GetValue("TAB1",  "TAB1_A41");
						НоваяСтрока.ЕдИзмМедок=Отчет.GetValue("TAB1",  "TAB1_A4");
// Серна - Конец
						//20%
						...
						ИначеЕсли  Отчет.GetValue("TAB1", "TAB1_A011")=7 Тогда
							...
							КонецЕсли;
						//14%
						ИначеЕсли  Отчет.GetValue("TAB1", "TAB1_A011")=14 Тогда
							НоваяСтрока.СтавкаНДС  =  Перечисления.СтавкиНДС.НДС14;
							Новаястрока.Сумма  = Отчет.GetValue("TAB1",  "TAB1_A013");
							Новаястрока.СуммаНДС = Новаястрока.Сумма * 0.14;
							НоваяСтрока.СуммаВзаиморасчетов = Новаястрока.Сумма + Новаястрока.СуммаНДС;
...
// Серна - Конец
			...
			ИначеЕсли Лев(ИмяСхемы,  7) = "1С82АВР" Тогда
				...
				Попытка
					...
					Если НЕ ЗначениеЗаполнено(Покупатель) Тогда 
						...
						Иначе
							МедКомпонент.CancelDoc("Акт " + СокрЛП(Акт.НомерВходящегоДокумента) + " від " + Формат(Акт.ДатаВходящегоДокумента,  "ДЛФ = Д") 
													+ Символы.ПС + "Контрагента з ІПН = " + КонтрИНН + " та ЄДРПОУ = " +КонтрЕДРПОУ+" не існує в 1С!" + Символы.ПС 
													+ "Створення документу неможливо!"  + Символы.ПС + "Увімкніть функцію автоматичного створення контрагента!");
							
							Сообщение = Новый СообщениеПользователю();
							Сообщение.Текст = ("Акт " + СокрЛП(Акт.НомерВходящегоДокумента) + " від " + Формат(Акт.ДатаВходящегоДокумента,  "ДЛФ = Д") 
													+ Символы.ПС + "Контрагента з ІПН = " + КонтрИНН + " та ЄДРПОУ = " +КонтрЕДРПОУ+" не існує в 1С!" + Символы.ПС 
													+ "Створення документу неможливо!"  + Символы.ПС + "Увімкніть функцію автоматичного створення контрагента!");
							Сообщение.Сообщить();
							Возврат;
				...
				Попытка
					...
					Рах.ВидОперации = Перечисления.ВидыОперацийСчетНаОплатуПоставщика.ПокупкаКомиссия;
					ВидОперации = Рах.ВидОперации;
						//ИначеЕсли ИмяБазы="УПП" ИЛИ ИмяБазы="УТ" Тогда
					КонецЕсли;
					...
					Рах.ДатаВходящегоДокумента  =  ДатаДокаВх;
					Рах.НомерВходящегоДокумента =  НомерДокаВх; 
					Рах.Дата  =  Дата(ТекущаяДата()); 
					Рах.ВалютаДокумента = Справочники.Валюты.НайтиПоКоду("980");
					ЗаполнениеДокументов.ЗаполнитьШапкуДокумента(Рах,  ПараметрыСеанса.ТекущийПользователь);
					//-----------------------------------------------------  
					ЕДРПОУОрганизации  =  Строка(СокрЛП(Отчет.GetValue("MAIN",  "SIDE_EDRPOU_K")));
					Если НЕ ЗначениеЗаполнено(ЕДРПОУОрганизации) Тогда
						МедКомпонент.CancelDoc("В рахунку не вказаний ЄДРПОУ!");
						Возврат;
					КонецЕсли; 
					...
					Если НЕ ЗначениеЗаполнено(Покупатель) Тогда 
						...
						Если СозданиеКонтрагентов Тогда
							Покупатель = СозданиеКонтрагента(Отчет, НазваниеДока, НомерДокаВх, ДатаДокаВх, ФилиалВхДок);	
						Иначе
							МедКомпонент.CancelDoc("Рахунок " + СокрЛП(Рах.НомерВходящегоДокумента) + " від " + Формат(Рах.ДатаВходящегоДокумента,  "ДЛФ = Д") 
													+ Символы.ПС + "Контрагента з ІПН = " + КонтрИНН + " та ЄДРПОУ = " +КонтрЕДРПОУ+" не існує в 1С!" + Символы.ПС 
													+ "Створення документу неможливо!"  + Символы.ПС + "Увімкніть функцію автоматичного створення контрагента!");
							
							Сообщение = Новый СообщениеПользователю();
							Сообщение.Текст = ("Рахунок " + СокрЛП(Рах.НомерВходящегоДокумента) + " від " + Формат(Рах.ДатаВходящегоДокумента,  "ДЛФ = Д") 
													+ Символы.ПС + "Контрагента з ІПН = " + КонтрИНН + " та ЄДРПОУ = " +КонтрЕДРПОУ+" не існує в 1С!" + Символы.ПС 
													+ "Створення документу неможливо!"  + Символы.ПС + "Увімкніть функцію автоматичного створення контрагента!");
							Сообщение.Сообщить();
							Возврат;
					...
					КонецЕсли;       				
					//-----------------------------------------------------
					Рах.Комментарий="Переданий з M.E.Doc";
					//Если ИмяБазы="УТ" или ИмяБазы="УТП"  Тогда
					//	Рах.ОтражатьВБухгалтерскомУчете=Истина;
					//КонецЕсли;
					//-----------------------------------------------------
					table  =  Отчет.First("TAB1"); 
					Пока table Цикл
						...
						НомКоличество = Отчет.GetValue("TAB1",  "TAB1_A3");
						НомЦена = Отчет.GetValue("TAB1",  "TAB1_A5");
						Если ИмяСхемы = "1С82РАХ" Тогда
							НомЦена = НомЦена;
							НомСтавкаНДС = Перечисления.СтавкиНДС.НДС20;
							СуммаНДС = НомЦена * 0.2;
						ИначеЕсли ИмяСхемы = "1С82РАХБ" Тогда
							НомЦена = НомЦена;
							НомСтавкаНДС = Перечисления.СтавкиНДС.БезНДС;
							СуммаНДС = 0;
						ИначеЕсли ИмяСхемы = "1С82РАХ0" Тогда
						...
						Если ЗначениеЗаполнено(НоменклатураДока) Тогда
							НоваяСтрока =  Рах.Товары.Добавить();
							НомДобавить(НоваяСтрока,  НоменклатураДока,  НомКоличество,  НомЦена,  НомСумма,  НомСтавкаНДС, СуммаНДС,ЕдИзмНоменклатуры);
						иначе
					...
					МедКомпонент.AcceptDoc(пGUID); 
					Форма  =  Рах.ПолучитьФорму("ФормаДокумента");
			...
			ИначеЕсли Лев(ИмяСхемы,  6) = "1С82ВН" Тогда
				...
				Попытка
					...
					Если НЕ ЗначениеЗаполнено(Покупатель) Тогда 
						...
						Иначе
							МедКомпонент.CancelDoc("Видаткова накладна " + СокрЛП(Акт.НомерВходящегоДокумента) + " від " + Формат(Акт.ДатаВходящегоДокумента,  "ДЛФ = Д") 
													+ Символы.ПС + "Контрагента з ІПН = " + КонтрИНН + " та ЄДРПОУ = " +КонтрЕДРПОУ+" не існує в 1С!" + Символы.ПС 
													+ "Створення документу неможливо!"  + Символы.ПС + "Увімкніть функцію автоматичного створення контрагента!");
							
							Сообщение = Новый СообщениеПользователю();
							Сообщение.Текст = ("Видаткова накладна " + СокрЛП(Акт.НомерВходящегоДокумента) + " від " + Формат(Акт.ДатаВходящегоДокумента,  "ДЛФ = Д") 
													+ Символы.ПС + "Контрагента з ІПН = " + КонтрИНН + " та ЄДРПОУ = " +КонтрЕДРПОУ+" не існує в 1С!" + Символы.ПС 
													+ "Створення документу неможливо!"  + Символы.ПС + "Увімкніть функцію автоматичного створення контрагента!");
							Сообщение.Сообщить();
							Возврат;
			...
			ИначеЕсли Лев(ИмяСхемы,  7) = "1С77АВР" Тогда	
				...
				Попытка
					...
					КонецЕсли;
						НоменклатураДока = Справочники.Номенклатура.НайтиПоНаименованию(НоменклатураОригинал,  Истина);
						НомКоличество = Отчет.GetValue("TAB1",  "TAB1_A3");
						Если  Отчет.GetValue("TAB1",  "TAB1_A3") <> Неопределено Тогда 
			...
			ИначеЕсли Лев(ИмяСхемы,  6) = "1С77ВН" Тогда	
				...
				Попытка
					ВН  =  Документы.ПоступлениеТоваровУслуг.СоздатьДокумент();
					Если ИмяБазы = "БУ" 
						ИЛИ ИмяБазы = "УТП" 
						ИЛИ ИмяБазы = "кпЖРЕУ" 
						ИЛИ ИмяБазы = "БухгалтерияСельскохозяйственногоПредприятия" 
						...
						ВН.ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.ПокупкаКомиссия;
						ВидОперации = ВН.ВидОперации;
						//ИначеЕсли ИмяБазы="УПП" ИЛИ ИмяБазы="УТ" Тогда
					КонецЕсли;
					...
					ВН.ДатаВходящегоДокумента = ДатаДокаВх;
					ВН.НомерВходящегоДокумента = НомерДокаВх; 
					ВН.Дата = Дата(ТекущаяДата()); 
					ВН.ВалютаДокумента = Справочники.Валюты.НайтиПоКоду("980");
					ЗаполнениеДокументов.ЗаполнитьШапкуДокумента(ВН,  ПараметрыСеанса.ТекущийПользователь);
					//-----------------------------------------------------  
					ЕДРПОУОрганизации  =  Строка(СокрЛП(Отчет.GetValue("MAIN",  "SIDE_EDRPOU_K")));
					Если НЕ ЗначениеЗаполнено(ЕДРПОУОрганизации) Тогда
						МедКомпонент.CancelDoc("В Видатковій накладній не вказаний ЄДРПОУ!");
						Возврат;
					КонецЕсли; 
					...
					Если НЕ ЗначениеЗаполнено(Покупатель) Тогда 
						...
						Если СозданиеКонтрагентов Тогда
							Покупатель = СозданиеКонтрагента(Отчет, НазваниеДока, НомерДокаВх, ДатаДокаВх, "");	
						Иначе
							МедКомпонент.CancelDoc("Видаткова накладна " + СокрЛП(ВН.НомерВходящегоДокумента) + " від " + Формат(ВН.ДатаВходящегоДокумента,  "ДЛФ = Д") 
							+ Символы.ПС + "Контрагента з ІПН = " + КонтрИНН + " та ЄДРПОУ = " +КонтрЕДРПОУ+" не існує в 1С!" + Символы.ПС 
													+ "Створення документу неможливо!"  + Символы.ПС + "Увімкніть функцію автоматичного створення контрагента!");
							
							Сообщение = Новый СообщениеПользователю();
							Сообщение.Текст = ("Видаткова накладна " + СокрЛП(ВН.НомерВходящегоДокумента) + " від " + Формат(ВН.ДатаВходящегоДокумента,  "ДЛФ = Д") 
													+ Символы.ПС + "Контрагента з ІПН = " + КонтрИНН + " та ЄДРПОУ = " +КонтрЕДРПОУ+" не існує в 1С!" + Символы.ПС 
													+ "Створення документу неможливо!"  + Символы.ПС + "Увімкніть функцію автоматичного створення контрагента!");
							Сообщение.Сообщить();
							Возврат;
					...
					КонецЕсли;       				
					//-----------------------------------------------------
					ВН.Комментарий="Переданий з M.E.Doc";
					Если ИмяБазы="УТ" Тогда
						ВН.ОтражатьВБухгалтерскомУчете=Истина;
					КонецЕсли;
					...
					Пока table Цикл 
						...
						Если ИмяСхемы = "1С77ВН" Тогда
							...
							НомСтавкаНДС = Перечисления.СтавкиНДС.НДС20;
							СуммаНДС = НомСумма * 0.2;	
							ВН.СуммаВключаетНДС = Ложь;
						ИначеЕсли ИмяСхемы = "1С77ВНБ" Тогда				
							НомЦена = НомЦена;
							НомСтавкаНДС = Перечисления.СтавкиНДС.БезНДС;
							ВН.СуммаВключаетНДС = ЛОЖЬ;
							СуммаНДС = 0;
						   // ВН.УчитыватьНДС = Ложь;				
						КонецЕсли; 
						ЕИОригинал = СокрЛП(Отчет.GetValue("TAB1",  "TAB1_NOMENKLATURA_CHARCODEUMEASURE"));
						ЕдИзмНоменклатуры  =  Справочники.КлассификаторЕдиницИзмерения.НайтиПоНаименованию(ЕИОригинал);   
						
						Если ЗначениеЗаполнено(НоменклатураДока) Тогда
							НоваяСтрока =  ВН.Товары.Добавить();
							НомДобавить(НоваяСтрока, НоменклатураДока,  НомКоличество,  НомЦена,  НомСумма,  НомСтавкаНДС, СуммаНДС, ЕдИзмНоменклатуры);
						иначе
					...
					МедКомпонент.AcceptDoc(пGUID); 
					Форма  =  ВН.ПолучитьФорму("ФормаДокумента");
					Если ЭтаФорма.НЕоткрыватьВхДок = Ложь
					Тогда 
						Форма.Открыть();
					КонецЕсли;
			...
			//-----------------------------------------------------------------------------------------------------------------------------------------------//  
					Если НЕ ЗначениеЗаполнено(Покупатель) Тогда 
						...
						Если СозданиеКонтрагентов Тогда
							Покупатель = СозданиеКонтрагента(Отчет, НазваниеДока, НомерДокаВх, ДатаДокаВх, "");	
						Иначе
							МедКомпонент.CancelDoc("ТТН " + СокрЛП(ТТН.НомерВходящегоДокумента) + " від " + Формат(ТТН.ДатаВходящегоДокумента,  "ДЛФ = Д") 
							+ Символы.ПС + "Контрагента з ІПН = " + КонтрИНН + " та ЄДРПОУ = " +КонтрЕДРПОУ+" не існує в 1С!" + Символы.ПС 
													+ "Створення документу неможливо!"  + Символы.ПС + "Увімкніть функцію автоматичного створення контрагента!");
							
							Сообщение = Новый СообщениеПользователю();
							Сообщение.Текст = ("ТТН " + СокрЛП(ТТН.НомерВходящегоДокумента) + " від " + Формат(ТТН.ДатаВходящегоДокумента,  "ДЛФ = Д") 
													+ Символы.ПС + "Контрагента з ІПН = " + КонтрИНН + " та ЄДРПОУ = " +КонтрЕДРПОУ+" не існує в 1С!" + Символы.ПС 
													+ "Створення документу неможливо!"  + Символы.ПС + "Увімкніть функцію автоматичного створення контрагента!");
							Сообщение.Сообщить();
							Возврат;
...
Процедура ТаблицаДокументовПриАктивизацииСтроки(Элемент)
		 ...
		 ВыбраноБезНДСЧисло = 0;
		 ВыбраноВсегоНДСЧисло = 0;
		 ВыбраноВсегоЧисло = 0;
	...
	Для каждого СтрокаДокумент Из ЭлементыФормы.ТаблицаДокументов.ВыделенныеСтроки Цикл 
				ВыделеноБезНДС = ВыделеноБезНДС + СтрокаДокумент.СуммаБезНДС;
				ВыделеноВсегоНДС  = ВыделеноВсегоНДС + СтрокаДокумент.СуммаНДСДокумента;
				ВыделеноСумма =  ВыделеноСумма + СтрокаДокумент.СуммаДокумента;
	КонецЦикла; 
	
		 ВыбраноБезНДСЧисло = ВыделеноБезНДС;
		 ВыбраноВсегоНДСЧисло =  ВыделеноВсегоНДС;
		 ВыбраноВсегоЧисло = ВыделеноСумма;
...
Процедура ДобавитьСписокПервички()
	...
	Если Орг1С = "32912338" Тогда
		СписокПервички.Добавить("РеализацияУслугПоПереработке",  Нстр("ru = 'Акт выполненных работ (упр.)';uk = 'Акт виконаних послуг (упр.)'"));
	КонецЕсли;	
...
Процедура ПередаемАктСвойШаблон(App,  pd,  OrgId,  Документ,  Бланк,  ИД, КоличествоДокументовАкт, НайденнаяСтрока)
	
	Попытка
		Формирование_АктСвойШаблон(App,  pd,  OrgId,  Документ,  Бланк,  ИД); 				
		КоличествоДокументовАкт  =  КоличествоДокументовАкт+1;
		ПроверкаСтатусов(App, Документ, ИД);
		СписокДляОткрытия.Добавить(ИД);
	Исключение
...
Процедура ПередаемАктСвойШаблон_AktRobit(App,  pd,  OrgId,  Документ,  Бланк,  ИД, КоличествоДокументовАкт, НайденнаяСтрока)
	
	Попытка
		Формирование_АктСвойШаблон_AktRobit(App,  pd,  OrgId,  Документ,  Бланк,  ИД); 				
		КоличествоДокументовАкт  =  КоличествоДокументовАкт+1;
		ПроверкаСтатусов(App, Документ, ИД);
		СписокДляОткрытия.Добавить(ИД);
	Исключение
...
Процедура ПередаемАктСвойШаблон_AKT_PS(App,  pd,  OrgId,  Документ,  Бланк,  ИД, КоличествоДокументовАкт, НайденнаяСтрока)
	
	Попытка
		Формирование_АктСвойШаблон_AKT_PS(App,  pd,  OrgId,  Документ,  Бланк,  ИД); 				
		КоличествоДокументовАкт  =  КоличествоДокументовАкт+1;
		ПроверкаСтатусов(App, Документ, ИД);
		СписокДляОткрытия.Добавить(ИД);
	Исключение
...
// Серна - Начало
	СозданныеМедком = Строка("Новые (клиент-банк)");
// Серна - Конец
...
Процедура ОбновлениеСтатусаРНН() 
	...
	Попытка
		App = Новый COMОбъект("Medoc.ZApplication")
	Исключение
		Предупреждение(НСтр("ru = 'Программа M.E.Doc не запущена или не выполнен вход в предприятие.'; uk = 'Програму M.E.Doc не запущено або не виконано вхід у підприємство.'"));
		Возврат;
	КонецПопытки;	
	...
	Для каждого СтрокаДокумент Из ТаблицаДокументов Цикл 
		...
		Документ = СтрокаДокумент.Документ; 
		ИдДокумента1С = Строка(Документ.УникальныйИдентификатор()); 
		
		Если СтрокаДокумент.Пометка Тогда
			
			НомерДокумента1С = ПолучитьНомерБезНулей(Документ);
			...
			Контрагент1С = СокрЛП(СтруктураПараметров.НалоговыйДокумент.HKBUY);
			...
			Исключение
				Дата = Дата(Документ.Дата); 
				Reestr = prim.GetPrimaryReestr(docType, 1, Дата, Дата);
			КонецПопытки;	
			Reestr.First();
			Счетчик = Reestr.RecordCount;
			Пока Счетчик <> 0 Цикл
				ИдДокументаМед = Reestr.Fields.Item("EXDOC_ID").Value;
				НомерДокументаМед = Reestr.Fields.Item("DOC_NUM").Value;
				КонтрагентМед = Reestr.Fields.Item("PATRNER_IPN").Value;
				
// Серна - Начало
				//Если ИдДокумента1С = ИдДокументаМед и НомерДокумента1С = НомерДокументаМед и Контрагент1С = КонтрагентМед Тогда
				НомерРегистрацииВЕРПН = Reestr.Fields.Item("SEND_DPA_RN").Value;
				Если НомерДокумента1С = НомерДокументаМед и КонтрагентМед = Контрагент1С Тогда
					Если ИдДокумента1С = ИдДокументаМед Тогда
// Серна - Конец
					НаличиеВМедке = Истина;
					ДатаРегистрации = Reestr.Fields.Item("REGDATE").Value; 
					НомерРегистрации = Reestr.Fields.Item("REG_NUM").Value;
					Если ЗначениеЗаполнено(ДатаРегистрации) и Документ.ВключенаВЕдиныйРеестрНалоговыхНакладных = Ложь Тогда
						Документ1С = Документ.ПолучитьОбъект();
						Документ1С.ВключенаВЕдиныйРеестрНалоговыхНакладных = Истина;
// Серна - Начало
						Документ1С.ДатаРегистрации = НачалоДня(ДатаРегистрации);
						Документ1С.НомерРегистрацииВЕРПН = СокрЛП(НомерРегистрации);
// Серна - Конец
						Если НЕЗаполнятьКомментарии = Ложь Тогда
// Серна - Начало
							//Документ1С.Комментарий = Строка("Дата регистрации документа - "+Формат(ДатаРегистрации,  "ДФ = dd.MM.yyyy")+", регистрационний номер - "+НомерРегистрации);
// Серна - Конец
						КонецЕсли;						
						Документ1С.Записать();
						СтрокаДокумент.ВключенаВЕдиныйРеестрНалоговыхНакладных = Истина; 
					ИначеЕсли ЗначениеЗаполнено(ДатаРегистрации) и Документ.ВключенаВЕдиныйРеестрНалоговыхНакладных = Истина Тогда	
						Сообщить("Для документа " +Документ+ " уже установлен признак фактической регистрации в ЕРНН.");
					КонецЕсли;
					
// Серна - Начало
					ИначеЕсли (Документ.Метаданные().Имя = "НалоговаяНакладная") и (ИдДокументаМед = "") и (Reestr.Fields.Item("SENDER").Value = 1) и (Reestr.Fields.Item("DOC_MOVE").Value = 1)
						и (Reestr.Fields.Item("VATSUM").Value = СтрокаДокумент.СуммаНДСДокумента) и ( НачалоДня(Reestr.Fields.Item("DOC_DATE").Value) = НачалоДня(Документ.Дата) ) Тогда
						...
						Если ЗначениеЗаполнено(ЕРНН) и Документ.ВключенаВЕдиныйРеестрНалоговыхНакладных = Ложь Тогда
							Попытка
								...
								Док.ДатаРегистрации = НачалоДня(Reestr.Fields.Item("REGDATE").Value);
								Док.НомерРегистрацииВЕРПН = СокрЛП(Reestr.Fields.Item("REG_NUM").Value);
...
// Серна - Конец
			...
			Если НаличиеВМедке <> Истина Тогда 
				Сообщить("Документ " +Документ+ " не существует в M.E.Doc либо ещё не зарегистрирован в ЕРНН.");
			КонецЕсли;	
			
		КонецЕсли;	
		
		Если App.GetSendSTTByExDocID(ИдДокумента1С) <> -1 Тогда
			СтатусТекст = СокрЛП(ПолучитьРасшифровкуСтатуса(Строка(App.GetSendSTTByExDocID(ИдДокумента1С)))); 	
			СтатусТекст2 = СокрЛП(ПроверитьСтатусПоИД(Строка(App.GetDocStatus(ИдДокумента1С))));
			СтрокаДокумент.Статус = Формат(Строка(СтатусТекст + ",  " + СтатусТекст2));
		ИначеЕсли (НЕ ТипДок = "ВхНалоговыйДокумент") 
			И (НЕ ТипДок = "ВхПостТовУслуг") 
			И (НЕ ТипДок = "ВхСчет") 
			И App.GetInTrashStatus(ИдДокумента1С) = Ложь Тогда
			СтрокаДокумент.Статус = Формат(НСтр("ru = 'Документ не передавался в M.E.Doc';uk = 'Документ не передавався в M.E.Doc'")); 					
		ИначеЕсли App.GetInTrashStatus(ИдДокумента1С) = Истина	Тогда
			СтрокаДокумент.Статус = Формат(НСтр("ru = 'Документ в корзине M.E.Doc';uk = 'Документ у корзині M.E.Doc'"));			
		Иначе
			СтрокаДокумент.Статус = Формат(НСтр("ru = 'Документ не передавался в M.E.Doc';uk = 'Документ не передавався в M.E.Doc'"));
		КонецЕсли;     		
...
Функция Скрытие(БезНДСЧисло, ВсегоНДСЧисло, ВсегоЧисло1, ВыбраноБезНДСЧисло, ВыбраноВсегоНДСЧисло, ВыбраноВсегоЧисло)
	...
	Если ВыбраноБезНДСЧисло = 0 тогда
		ЭлементыФормы.ВыбраноБезНДСЧисло.Видимость = Ложь;
		ЭлементыФормы.Надпись26.Видимость = Ложь;
	Иначе
		ЭлементыФормы.ВыбраноБезНДСЧисло.Видимость = Истина;
		ЭлементыФормы.Надпись26.Видимость = Истина;
	КонецЕсли;
	Если ВыбраноВсегоНДСЧисло = 0 тогда
		ЭлементыФормы.ВыбраноВсегоНДСЧисло.Видимость = Ложь;
		ЭлементыФормы.Надпись27.Видимость = Ложь;
	Иначе	
		ЭлементыФормы.ВыбраноВсегоНДСЧисло.Видимость = Истина;
		ЭлементыФормы.Надпись27.Видимость = Истина;		
	КонецЕсли;
	Если ВыбраноВсегоЧисло = 0 тогда
		ЭлементыФормы.ВыбраноВсегоЧисло.Видимость = Ложь;
		ЭлементыФормы.Надпись28.Видимость = Ложь;
	Иначе
		ЭлементыФормы.ВыбраноВсегоЧисло.Видимость = Истина;
		ЭлементыФормы.Надпись28.Видимость = Истина;		
	КонецЕсли;	
КонецФункции
...
Процедура КоманднаяПанельТаблицаДокументовПередатьСвойШаблон(Кнопка)
	...
	Для каждого СтрокаДокумент Из ТаблицаДокументов Цикл  
		Если СтрокаДокумент.Пометка Тогда
			...
			ИначеЕсли ПерезаписьДокументов = "Нет" Тогда 
				...
				КонецЕсли; 
			КонецЕсли;			
	Если Орг1С = "39404434" Тогда		
			Если ТипДок = "ОказаниеУслуг" Тогда 
				Бланк  =  "UCAKT";
				ПередаемАктСвойШаблон(App, pd, OrgId, Документ, Бланк, ИД, КоличествоДокументовАкт, НайденнаяСтрока);
			КонецЕсли; 
	КонецЕсли;
	Если Орг1С = "39058283" Тогда		
			Если ТипДок = "ОказаниеУслуг" Тогда 
				Бланк  =  "AktRobit";
				ПередаемАктСвойШаблон_AktRobit(App, pd, OrgId, Документ, Бланк, ИД, КоличествоДокументовАкт, НайденнаяСтрока);
			КонецЕсли; 
	КонецЕсли;
	Если Орг1С = "32912338" Тогда		
			Если ТипДок = "РеализацияУслугПоПереработке" Тогда 
				Бланк  =  "AKT_PS";
				ПередаемАктСвойШаблон_AKT_PS(App, pd, OrgId, Документ, Бланк, ИД, КоличествоДокументовАкт, НайденнаяСтрока);
			КонецЕсли; 
	КонецЕсли;	
	...
	КоличествоДокументов =  КоличествоДокументовАкт;  	
...
// ПроверитьДокументВ1С
//
Функция ПроверитьДокументВ1С(Документ,Организация, НомерРегВх, СпецРежимДляНомераВх = "", пДатаДокумента)
	...
	| 	И Док.НомерРегистрацииВЕРПН	=	&НомерРегистрацииВЕРПН
	| 	И Док.СпецРежимНалогообложения1=	&СпецРежимНалогообложения1
	|	И Док.Дата >= НАЧАЛОПЕРИОДА(&ДатаДокумента, ГОД)
	|	И Док.Дата <= КОНЕЦПЕРИОДА(&ДатаДокумента, ГОД)
	|";
	
	Запрос.УстановитьПараметр("Организация",  Организация);
	Запрос.УстановитьПараметр("НомерРегистрацииВЕРПН",  НомерРегВх);
	Запрос.УстановитьПараметр("ДатаДокумента",  пДатаДокумента);
	
	лСпецРежимТут = СокрЛП(СпецРежимДляНомераВх);
	Если лСпецРежимТут = "" Тогда лСпецРежимТут = 0;
	Иначе
		Попытка
			лСпецРежимТут = Число(лСпецРежимТут);
		Исключение
			лСпецРежимТут = 0;
		КонецПопытки;
	КонецЕсли;
	Запрос.УстановитьПараметр("СпецРежимНалогообложения1",  лСпецРежимТут);
	Выборка  =  Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда 
		Возврат Истина;
	Иначе
...
КонецФункции // ПроверитьДокументВ1С
...
СписокРеглОтчетов.Добавить("РегламентированныйОтчетУточняющийРасчетДекларацияНДС",  Нстр("ru = 'Уточняющий расчет Декларация по НДС';uk = 'Утоянюючий розрахунок Декларація по ПДВ'"));
